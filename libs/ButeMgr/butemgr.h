...
private:
	...
	typedef stdext::hash_set< std::string > StringHolder;

	// Used to define map of strings to CSymTabItems.
	typedef stdext::hash_map< char const*, CSymTabItem*, hash_str_nocase > TableOfItems;

	// Used to define map of strings to TableOfItems.
	typedef stdext::hash_map< char const*, TableOfItems*, hash_str_nocase > TableOfTags;
	...
	const std::string & AddString( char const* pszString );
