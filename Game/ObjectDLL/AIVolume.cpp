...
AINode* AIVolumePlayerInfo::FindViewNode(CAI* pAI, EnumAINodeType eNodeType, const LTVector& vPos, LTBOOL bMustBeUnowned, HOBJECT hThreat) const
{
	...
	for ( uint32 iViewNode = 0 ; iViewNode < kMaxViewNodes ; iViewNode++ )
	{
		...
		if ( pNode && ( ( pNode->GetType() == eNodeType ) || pNode->NodeTypeIsActive( eNodeType ) ) )
		{
			...
			if( hThreat && ( pNode->GetStatus( pAI, vPos, hThreat ) != kStatus_Ok ) )
			{
				continue;
			}
...
AINode* AIVolumePlayerInfo::FindOwnedViewNode(CAI* pAI, EnumAINodeType eNodeType, HOBJECT hOwner, HOBJECT hThreat) const
{
	// Get AIs Path Knowledge.
	...
	for ( uint32 iViewNode = 0 ; iViewNode < kMaxViewNodes ; iViewNode++ )
	{
		...
		if ( pNode && ( ( pNode->GetType() == eNodeType ) || pNode->NodeTypeIsActive( eNodeType ) ) )
		{
			...
			if( hThreat && ( pNode->GetStatus( pAI, pAI->GetPosition(), hThreat ) != kStatus_Ok ) )
			{
				continue;
			}
