...
void CAIPathMgr::BuildPath(CAI* pAI, CAIPath* pPath, AIVolume* pVolume, const LTVector& vPosDest)
{
	...
	**/
	...
	/***
	AITRACE( AIShowPaths, ( pAI->m_hObject, "BOUNDED PATH?" ) );

	AIVolume* pContains = pAI->GetCurrentVolume();
	AIVolume* pTest;
	LTVector vTest;
	for( it = pPath->Begin(); it != pPath->End(); ++it )
	{
		pWaypt = *it;
		if( pWaypt->GetInstruction() != CAIPathWaypoint::eInstructionMoveTo )
		{
			continue;
		}

		vTest = pWaypt->GetArgumentVector1();
		pTest = g_pAIVolumeMgr->FindContainingVolume( pAI->m_hObject, vTest, eAxisAll, pAI->GetVerticalThreshold(), pContains );
		pContains = pTest;
		AITRACE( AIShowPaths, ( pAI->m_hObject, "   %.2f %.2f %.2f (%s)",
			vTest.x, vTest.y, vTest.z, 
			( pTest ? pTest->GetName() : "NONE" ) ) );
	}
	***/
}
...
void CAIPathMgr::BuildDoorPath(CAI* pAI, CAIPath* pPath, LTFLOAT fRadius, AIVolume* pVolume, const LTVector& vDir, CAIPathWaypoint* pWaypt, CAIPathWaypoint* pLastWaypt)
{
	...
	{
		Door* pDoor = (Door*)g_pLTServer->HandleToObject( pVolume->GetDoor(0) );
		LTVector vLastPos = pDoor->GetOriginalPos();

		vLastPos += vDir * ( fRadius * 2.f );
