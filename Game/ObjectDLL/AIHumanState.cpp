...
LTBOOL CAIHumanStateSearch::FindNode()
{
	...
	if ( pRegion && pRegion->IsSearchable() )
	{
		...
		AINodeSearch* pNode = pRegion->FindNearestSearchNode( GetAI(), GetAI()->GetPosition(), fCurTime);
		if ( !pNode )
		{
			return LTFALSE;
		}
...
void CAIHumanStateUseObject::Drive( float fYOffset )
{
	GetAI()->FaceDir( m_pUseNode->GetForward() );
	...
	vPos.y += fYOffset;
	g_pLTServer->SetObjectPos( GetAI()->m_hObject, &vPos );

	GetAI()->m_bDrive = true;
	GetAI()->SetMoveToFloor( false );
}
			...
			if ( m_pStrategyShoot->IsFiring() )
			{
				...
				m_pStrategyShoot->Update();
					  ...
					  ( ( pTarget->IsVisibleFromWeapon() ) ||
					    ( m_pStrategyFollowPath->StartedToOpenDoor() ) ) )
			{
				m_pStrategyShoot->Update();
			}
		}
...
LTBOOL CAIHumanStateChase::CalculatePath()
{
	...
	if( GetAI()->HasTarget() )
	{
		...
		// Do not use an offset if the target has exited the volumes.
		// This prevents AI from running in circles when the target
		// is outside fo the volumes.

		if( !GetAI()->GetTarget()->GetCharacter()->HasCurrentVolume() )
		{
			vOffset = LTVector( 0.f, 0.f, 0.f );
		}
	}
...
void CAIHumanStateAttack::Update()
{
	...
	// Throw grenades when target goes out of sight.

	if( ( !pTarget->IsVisibleCompletely() ) && 
		( m_pStrategyGrenade ) && 
		( m_aniPosture.GetProp() == kAP_Stand ) && 
		( m_pStrategyGrenade->ShouldThrow( 2.f ) ) )
	{
	...
	{
		if ( m_pStrategyGrenade && m_aniPosture.GetProp() == kAP_Stand && m_pStrategyGrenade->ShouldThrow( 1.f ) )
		{
			m_pStrategyGrenade->Throw();
		}
		else
...
void CAIHumanStateAttackFromCover::Update()
{
	...
	if ( m_bPlayFirstSound || m_bFirstUpdate )
	{
		if( m_bOrderedCover )
		{
			GetAI()->PlayCombatSound(kAIS_OrderedCover);
		}
		else {
...
void CAIHumanStateAttackFromCover::UpdateGotoCover()
{
	...
	if( m_pCoverNode )
	{
		EnumNodeStatus eCoverStatus = m_pCoverNode->GetStatus( GetAI(), GetAI()->GetPosition(), GetAI()->GetTarget()->GetObject());
...
void CAIHumanStateAttackFromCover::HandleDamage(const DamageStruct& damage)
{
	...
	// jeffo - 07/29/03
	// Ignore damage if movement is locked.
	// This was added to fix the bug where AI jump over a rail,
	// and hang in mid-air forever.
...
void CAIHumanStateAttackFromVantage::UpdateMoving()
{
	...
	if( m_pVantageNode )
	{
		EnumNodeStatus eVantageStatus = m_pVantageNode->GetStatus( GetAI(), GetAI()->GetPosition(), GetAI()->GetTarget()->GetObject());
...
void CAIHumanStateCharge::Update()
{
	...
	// Charge was only called to get the AI to move from
	// an invalid node (e.g. cover), so stop if the AI has
	// move a body-width away.

	if( m_bChargeFromNode )
	{
		LTFLOAT fWidth = GetAI()->GetDims().x * 2.f;
		LTFLOAT fDistSqr = m_vStartPos.DistSqr( GetAI()->GetPosition() );
		if( fDistSqr > fWidth * fWidth )
		{
			if( !GetAI()->GetAIMovement()->IsMovementLocked() )
			{
				m_eStateStatus = kSStat_StateComplete;
	...
	else if( !GetAI()->GetAIMovement()->IsMovementLocked() )
	{
...
void CAIHumanStateCharge::Load(ILTMessage_Read *pMsg)
{
	...
	LOAD_VECTOR(m_vStartPos);
	LOAD_BOOL(m_bChargeFromNode);
}
...
void CAIHumanStateCharge::Save(ILTMessage_Write *pMsg)
{
	...
	SAVE_VECTOR(m_vStartPos);
	SAVE_BOOL(m_bChargeFromNode);
}
