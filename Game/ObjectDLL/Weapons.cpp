...
LTBOOL CWeapons::Init(HOBJECT hOwner, HOBJECT hWeaponModel)
{
    LTBOOL bRet = LTFALSE;
...
void CWeapons::CreateWeapon(uint8 nWeaponId, uint8 nAmmoId)
{
	...
	if (pWeapon)
	{
		pWeapon->Init(this, m_hOwner, nWeaponId, nAmmoId);
...
LTBOOL CWeapons::AddWeapon(LPBASECLASS pObject, HOBJECT hSender, ILTMessage_Read *pMsg)
{
	...
	// Check if we should do weaponsstay.  This means that you can't pickup a weaponitem
	// if you already have that weapon.
	if( IsMultiplayerGame( ))
	{
		if( g_pServerMissionMgr->GetServerSettings( ).m_bWeaponsStay )
				...
				if (IsPlayer(m_hOwner))
				{
					...
					if (pPlayer)
					{
						...
						if (hClient)
						{
							...
							cMsg.Writebool( pPlayer->IsActivatingPickup( ));
						   ...
						   uint8 nAmmoId, int nAmmo, bool bForcePickup)
{
	...
	if (bPickedUp || bForcePickup)
	{
		...
		if (pObject)
		{
			// Check if we should do weaponsstay.  This means that weapons aren't removed
			// when they are picked up.
			bool bWeaponsStay = false;
			if( IsMultiplayerGame( ))
			{
				if( g_pServerMissionMgr->GetServerSettings( ).m_bWeaponsStay )
					if( m_pWeapons && m_pWeapons[nWeaponId] && m_pWeapons[nWeaponId]->Have( ))
...
LTBOOL CWeapons::AddAmmoBox(LPBASECLASS pObject, HOBJECT hSender, ILTMessage_Read *pMsg)
{
	...
	// Send the appropriate message to the client...
	// 08/05/03 - KEF - Don't send a PIC message unless we actually took some ammo.  (Send it anyway
	// in singleplayer to avoid breaking old behavior.)
	if (IsPlayer(m_hOwner) && (!IsMultiplayerGame() || bTookAmmo))
	{
        CPlayerObj* pPlayer = (CPlayerObj*)g_pLTServer->HandleToObject(m_hOwner);
        if (!pPlayer) return LTFALSE;
...
int CWeapons::AddAmmo(int nAmmoId, int nAmount)
{
	...
	int nMaxAmmo = pAmmo->GetMaxAmount(m_hOwner);
						 ...
						 bool bDisplayMsg /*=true*/)
{
	...
	if (bNotifyClient && IsPlayer(m_hOwner))
	{
       CPlayerObj* pPlayer = (CPlayerObj*)g_pLTServer->HandleToObject(m_hOwner);
		if (!pPlayer) return;
