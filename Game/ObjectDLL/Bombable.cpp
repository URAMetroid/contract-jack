...
void Bombable::SetupDisabledState( LTBOOL bDestroyed )
{
	...
	if( pGadgetTarget && pGadgetTarget->pszAmmoType )
		pAmmo = g_pWeaponMgr->GetAmmo( pGadgetTarget->pszAmmoType );
...
void Bombable::SetupDefusedState( )
{
	...
	// Check if we should do weaponsstay.  If so, then don't create a timebomb weaponitem
	// because it won't be able to be picked up.
	bool bWeaponsStay = IsMultiplayerGame( ) && g_pServerMissionMgr->GetServerSettings( ).m_bWeaponsStay;
	if( !bWeaponsStay )
	{
	...
	{
		// Bomb not created, clients can plant a bomb right away.
