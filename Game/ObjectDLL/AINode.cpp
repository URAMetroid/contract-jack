...
BEGIN_CLASS(AINodeCover)
	...
	ADD_REALPROP_FLAG(PlayerRadius,				768.0f,			0|PF_RADIUS)
...
void AINodeCover::ReadProp(ObjectCreateStruct* pocs)
{
    ...
    if ( g_pLTServer->GetPropGeneric( "PlayerRadius", &g_gp ) == LT_OK )
		if ( g_gp.m_String[0] )
			m_fPlayerRadiusSqr = g_gp.m_Float*g_gp.m_Float;
...
EnumNodeStatus AINodeCover::GetStatus( CAI* pAI, const LTVector& vPos, HOBJECT hThreat) const
{
	LTVector vThreatPos;
	...
	if ( VEC_DISTSQR(vPos, vThreatPos) < g_pAIButeMgr->GetSenses()->fThreatTooCloseDistanceSqr )
	{
		...
		if( pAI->GetAIMovement()->IsSet() )
		{
			vToNode = pAI->GetAIMovement()->GetDest() - vPos;
		}
	...
	if ( fDistSqr > m_fPlayerRadiusSqr )
	{
		return kStatus_TooFar;
	}
...
EnumNodeStatus AINodeVantage::GetStatus( CAI* pAI, const LTVector& vPos, HOBJECT hThreat) const
{
	LTVector vThreatPos;
    g_pLTServer->GetObjectPos(hThreat, &vThreatPos);
...
EnumNodeStatus AINodeSearch::GetStatus( CAI* pAI, const LTVector& vPos, HOBJECT hThreat) const
{
	if ( m_bSearched )
	{
...
EnumNodeStatus AINodeUseObject::GetStatus( CAI* pAI, const LTVector& vPos, HOBJECT hThreat) const
{
	LTVector vThreatPos;
    g_pLTServer->GetObjectPos(hThreat, &vThreatPos);
...
EnumNodeStatus AINodeBackup::GetStatus( CAI* pAI, const LTVector& vPos, HOBJECT hThreat) const
{
	LTVector vThreatPos;
    g_pLTServer->GetObjectPos(hThreat, &vThreatPos);
...
//              
//----------------------------------------------------------------------------
EnumNodeStatus AINodeObstruct::GetStatus( CAI* pAI, const LTVector& vPos, HOBJECT hThreat) const
{
	LTVector vOrigin;
...
/*virtual*/ void AINodeChangeWeapons::Verify()
{
	for ( unsigned int x = 0; x < m_WeaponSets.size(); x++ )
	{
		...
		g_pWeaponMgr->ReadWeapon( WeaponSet.m_szChangeToWeapon.c_str(), iNewWeapon, iNewAmmo );

		uint8 iRequiredAmmo, iRequiredWeapon;
		g_pWeaponMgr->ReadWeapon( WeaponSet.m_szChangeToWeaponRequirement.c_str(), iRequiredWeapon, iRequiredAmmo );
...
const AINodeChangeWeapons::ChangeWeaponSet* const AINodeChangeWeapons::GetWeaponTransition(CAIHuman* pAIHuman) const
{
	...
	for ( unsigned int x = 0; x < m_WeaponSets.size(); x++ )
	{
		...
		g_pWeaponMgr->ReadWeapon( pAIHuman->GetHolsterString(), iHolsteredWeapon, iHolsteredAmmo );
