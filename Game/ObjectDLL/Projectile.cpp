...
static CVarTrack g_vtVectorImpactDebug;
...
namespace
{
	void ProjectileInitFileGlobals()
	{
		...
		{
			g_vtVectorImpactDebug.Init(g_pLTServer, "VectorImpactDebug", LTNULL, 0.0f );
		}
	}
...
LTBOOL CProjectile::Setup(CWeapon const* pWeapon, WeaponFireInfo const & info)
{
	...
	if( fWeaponFireNoiseDistance > 0.f && IsCharacter( m_hFiredFrom ))
	{
...
void CProjectile::UpdateHeatSeeking( )
{
	...
	float fHeatSeekingRangeSqr = m_pAmmoData->fHeatSeekingRange * m_pAmmoData->fHeatSeekingRange;
	float fMaxAngle = MATH_DEGREES_TO_RADIANS( m_pAmmoData->fHeatSeekingAngle );

	CCharacter* pCharFiredFrom = dynamic_cast< CCharacter* >( g_pLTServer->HandleToObject( m_hFiredFrom ));

	CCharacter::CharacterList const& charList = CCharacter::GetCharacterList( );
	AimMagnet::ObjectList const& aimMagnetList = AimMagnet::GetObjectList( );
	lstObjectDists.reserve( charList.size( ) + aimMagnetList.size( ));
	...
	// Iterate thru all the aimmagnets and see if we should add them
	// to our target list.
	AimMagnet::ObjectList::const_iterator aimMagnetiter = aimMagnetList.begin( );
	for( ; aimMagnetiter != aimMagnetList.end( ); aimMagnetiter++ )
	{
		AimMagnet* pAimMagnet = const_cast< AimMagnet* >( *aimMagnetiter );

		g_pLTServer->GetObjectPos( pAimMagnet->m_hObject, &vTargetPos );
		objectDist.m_fDist = vTargetPos.DistSqr( vProjPos );
		...
		// Add him to the list.
		objectDist.m_pTarget = pAimMagnet;
		objectDist.m_bCharacter = false;
		lstObjectDists.push_back( objectDist );
	}
	...
	float fMaxTurn = MATH_DEGREES_TO_RADIANS( m_pAmmoData->fHeatSeekingRateOfTurn ) * g_pLTServer->GetFrameTime( );
...
void CProjectile::AddSpecialFX()
{
	...
	cSoundMsg.Writeuint8(m_pWeaponData->nId);
	cSoundMsg.Writeuint8(nShooterId);
	cSoundMsg.WriteCompPos(m_vFirePos);
	g_pLTServer->SendSFXMessage(cSoundMsg.Read(), m_vFirePos, 0);
}
...
bool DoVectorFilterFn(HOBJECT hObj, void *pUserData)
{
	...
	static HCLASS hCharacterHitBox = g_pLTServer->GetClass( "CCharacterHitBox" );
	static HCLASS hBody = g_pLTServer->GetClass( "Body" );
	static HCLASS hIntelligence = g_pLTServer->GetClass( "Intelligence" );
	static HCLASS hProjectile = g_pLTServer->GetClass( "CProjectile" );
	static HCLASS hWeaponItem = g_pLTServer->GetClass( "WeaponItem" );
	...
	if (ObjListFilterFn(hObj, pVectFilterFnUserData->m_pFilterList))
	{
		...
		if( g_pLTServer->IsKindOf( hObjClass, hCharacter ) || g_pLTServer->IsKindOf( hObjClass, hBody ) ||
			g_pLTServer->IsKindOf( hObjClass, hIntelligence ) || 
			g_pLTServer->IsKindOf( hObjClass, hProjectile ) || 
			g_pLTServer->IsKindOf( hObjClass, hWeaponItem ))
		{
            return false;
		}
                                     ...
                                     LTVector & vTo)
{
	...
	if (!bHitWorld && eSurfType != ST_LIQUID)
	{
		...
		WEAPON const *pWeapon = g_pWeaponMgr->GetWeapon(m_pWeaponData->nId);
		if( fDist > pWeapon->nEffectiveVectorRange )
		{
			// Calculate damage rolloff using the following form:
			// y = 1 / ( e^(k*x) ), where k is
			// ( -ln(.5f) / half_damage_dist )
			float fOutsideDist = fDist - pWeapon->nEffectiveVectorRange;
			float fK = ( 0.69314718f / pWeapon->fVectorHalfDamageDist );
			float fDamageFactor = 1.0f / ( float )exp( fK * fOutsideDist );
	...
	// KLS DEBUGGING of surface values made easy...
	if (g_vtVectorImpactDebug.GetFloat())
	{
		g_pLTServer->CPrint("Vector impacted surface: %s", pSurf->szName);
		g_pLTServer->CPrint("   Bullethole   = %s", pSurf->szBulletHoleSpr);
		g_pLTServer->CPrint("   ImpactFXName = %s", pSurf->szImpactFXName);
	}
