...
void CAIGoalAttackFromCover::SetStateAttack()
{
	...
	if( m_pAI->GetState()->GetStateType() != kState_HumanAttackFromCover )
	{
		...
		pAttackState->SetOrderedCover( m_bSeekTarget );
	}
}
...
void CAIGoalAttackFromCover::HandleStateUseObject()
{
	...
	EnumNodeStatus eCoverStatus = pCoverNode->GetStatus( m_pAI, m_pAI->GetPosition(), m_pAI->GetTarget()->GetObject() );
...
void CAIGoalAttackFromCover::CoverOrCharge()
{
	...
	pChargeState->SetChargeFromNode( LTTRUE );
...
AINode* CAIGoalAttackFromCover::FindNearestAttractorNode(EnumAINodeType eNodeType, const LTVector& vPos, LTFLOAT fDistSqr)
{
	...
	if( m_bSeekTarget )
	{
		...
		if( pPlayer )
		{
			...
			return g_pAINodeMgr->FindNearestNodeFromThreat(m_pAI, eNodeType, vPos, pPlayer->m_hObject, fSearchFactor);
		}
	}
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CAIGoalAttackFromCover::HandleNameValuePair
//
//	PURPOSE:	Handles getting a name/value pair.
//
// ----------------------------------------------------------------------- //

LTBOOL CAIGoalAttackFromCover::HandleNameValuePair(const char *szName, const char *szValue)
{
	...
	if ( !_stricmp(szName, "SEEKPLAYER") )
	{
		...
		{
			AITRACE( AIShowGoals, ( m_pAI->m_hObject, "CAIGoalAttackFromCover: SEEKPLAYER=1" ) );
	...
	return super::HandleNameValuePair( szName, szValue );
}
