...
enum EClientConnectionState
{
	...
	eClientConnectionState_Handshaking,
	eClientConnectionState_WaitingForAuth,
	eClientConnectionState_Connected,
};
...
class CGameServerShell : public IServerShellStub
{
	public :
		...
		void GenerateUniqueName( HCLIENT hClient, char const* pszNameBase, char* pszUniqueName, int nUniqueNameSize ) const;
	...
	protected :
		...
		// Checks if any client has the netname.
		bool		IsNameTaken( char const* pszName, HCLIENT hSkipClient ) const;
	...
	protected :
		...
		// Client told to self boot.  Flag him for removal later.
		void ClientSelfBoot( ClientData& clientData );
	...
	protected:
		...
		bool InitGameType( ServerGameOptions const& serverGameOptions );
