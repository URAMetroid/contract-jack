...
GrenadeStatus CAIBrain::GetGrenadeStatus( LTFLOAT fProbabilityFactor )
{
	if ( g_pLTServer->GetTime() >= m_fGrenadeStatusCheckTime )
	{
		m_fGrenadeStatusCheckTime = g_pLTServer->GetTime() + ( LOWER_BY_DIFFICULTY(m_pBrain->fAttackGrenadeThrowTime) / fProbabilityFactor );

		if ( GetRandom(0.0f, 1.0f) <= RAISE_BY_DIFFICULTY(m_pBrain->fAttackGrenadeThrowChance) * fProbabilityFactor )
		{
