...
BEGIN_CLASS(DisplayTimer)
	...
	ADD_STRINGPROP_FLAG( Type, "None", PF_STATICLIST)
...
CMDMGR_BEGIN_REGISTER_CLASS( DisplayTimer )
	...
	CMDMGR_ADD_MSG( TYPE,	2,	NULL,	"TYPE <0:none, 1:red, 2:blue, 3-5:bombtarget" )

CMDMGR_END_REGISTER_CLASS( DisplayTimer, GameBase )

static char const* s_paszDisplayTimerTypes[] =
{
	"None",
	"Team0", 
	"Team1", 
...
static uint32 const s_nNumDisplayTimerTypes = ARRAY_LEN( s_paszDisplayTimerTypes );
    ...
    const uint32 cMaxStringLength)
{
	...
	// Handle type...
	if( _stricmp( "Type", szPropName ) == 0 )
	{
		for( uint32 i = 0; i < s_nNumDisplayTimerTypes; i++ )
		{
			_ASSERT(cMaxStrings > (*pcStrings) + 1);
			strcpy( aszStrings[(*pcStrings)++], s_paszDisplayTimerTypes[i] );
		}
...
void DisplayTimer::ReadProp(ObjectCreateStruct *)
{
	...
	// Get the type this object belongs to.
	if( g_pLTServer->GetPropGeneric( "Type", &genProp ) == LT_OK && genProp.m_String[0] )
	{
		for( uint32 i = 0; i < s_nNumDisplayTimerTypes; i++ )
		{
			if( stricmp( genProp.m_String, s_paszDisplayTimerTypes[i] ) == 0 )
			{
				m_nType = i;
				break;
			}
...
bool DisplayTimer::OnTrigger(HOBJECT hSender, const CParsedMsg &cMsg)
{
	...
	else if (cMsg.GetArg(0) == s_cTok_Type)
	{
		if( cMsg.GetArgCount( ) > 1 )
		{
			uint32 nType = atoi( cMsg.GetArg( 1 ));
			if( nType >= s_nNumDisplayTimerTypes )
				nType = 0;
			m_nType = nType;
...
void DisplayTimer::UpdateClients()
{
...

	{
		...
		cMsg.Writebool(m_Timer.Paused());
		cMsg.Writeuint8( m_nType );
