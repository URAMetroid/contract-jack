...
//					Message		Num Params	Validation FnPtr		Syntax
	...
	CMDMGR_ADD_MSG( DRIVE,			2,		NULL,					"DRIVE <1 or 0> <y-offset>" )
	CMDMGR_ADD_MSG( DEATHPOP,		2,		NULL,					"DEATHPOP <1 or 0>" )
...
// Removed because it was it was prevent critical ai from entering the level.  CJ
// doesn't have waves of ai so this should be less of a problem than in NOLF2.
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CAI::SetProne()
//
//	PURPOSE:	Updates flags for prone AI
//
// ----------------------------------------------------------------------- //

void CAI::SetProne(bool bProne)
{

	if (bProne == m_bProne) return;
...
bool CAI::HandleCommand(const CParsedMsg &cMsg)
{
	...
	static CParsedMsg::CToken s_cTok_Drive("DRIVE");
	static CParsedMsg::CToken s_cTok_DeathPop("DEATHPOP");
	...
	else if( cMsg.GetArg(0) == s_cTok_Drive )
	{
		if ( cMsg.GetArgCount() > 1 )
		{	
			if( IsTrueChar(*cMsg.GetArg(1)) )
			{
				if( m_pState && m_pState->GetStateType() == kState_HumanUseObject )
				{
					CAIHumanStateUseObject* pStateUseObject = (CAIHumanStateUseObject*)m_pState;
					pStateUseObject->Drive( (float)atof( cMsg.GetArg(2) ) );
				}
			}
			else 
			{
	...
	else if( cMsg.GetArg(0) == s_cTok_DeathPop )
	{
		if ( cMsg.GetArgCount() > 1 )
		{	
			m_bDeathPop = !!IsTrueChar(*cMsg.GetArg(1));
		}
		return true;
	}
	else
...
void CAI::SetPosition(const LTVector& vPos, LTBOOL bFindFloorHeight)
{
	...
	float flHeight;
	if ( bFindFloorHeight && m_bMoveToFloor && ( LTTRUE == FindFloorHeight( m_vPos, &flHeight ) ) )
	{
...
void CAI::UpdatePosition()
{
	...
	g_pPhysicsLT->SetVelocity(m_hObject, &vVelocity);
  	...
  	if ( ( !m_pAIMovement->IsRotationLocked() ) && ( m_fRotationTimer < m_fRotationTime ) && ( !m_bDrive ) )
   	{
