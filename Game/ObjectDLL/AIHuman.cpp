...
void CAIHuman::DestroyArmor()
{
	...
	BaseClass* apObjects[AI_MAX_OBJECTS];
	CAttachmentPosition* apObjectPositions[AI_MAX_OBJECTS];
	// 08/08/03 - KEF - Ran into a case where this was NULL, but was unable to track down the source of the problem.
	// Filtering out the rest of the function seemed to behave properly, and seems better than crashing.
	if (!m_pAttachments)
		return;
	int cObjects = m_pAttachments->EnumerateObjects(apObjects, apObjectPositions, AI_MAX_OBJECTS);
...
void CAIHuman::UpdateMovement()
{
	...
	bool bDeadOnLadder = false;
	if( m_damage.IsDead() && m_pAnimationContext->IsPropSet( kAPG_Posture, kAP_Stand ) )
	{
		AITRACE( AIShowGoals, ( m_hObject, "DEAD FORCE GROUND: %d", m_bMoveToFloor ) );
		m_bForceGround = m_bMoveToFloor;

		AIVolume* pCurVolume = GetCurrentVolume();
		if( pCurVolume && pCurVolume->GetVolumeType() == AIVolume::kVolumeType_Ladder )
		{
			bDeadOnLadder = true;
		}
	}
	...
	else if ( m_bCheapMovement || m_bForceGround )
	{
		...
		if ( !bDeadOnLadder &&
			 !GetAnimationContext()->IsPropSet(kAPG_Movement, kAP_ClimbUp) && !GetAnimationContext()->IsPropSet(kAPG_Movement, kAP_ClimbDown) &&
...
// --------------------------------------------------------------------------- //
//
//	ROUTINE:	CAIHuman::SetMoveToFloor
//
//	PURPOSE:	Sets the move to floor flag and commands a move to floor.
//
// --------------------------------------------------------------------------- //

void CAIHuman::SetMoveToFloor( bool bValue )
{
