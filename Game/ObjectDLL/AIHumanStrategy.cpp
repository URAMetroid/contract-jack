			...
			switch ( pWaypoint->GetInstruction() )
			{
				...
				case CAIPathWaypoint::eInstructionOpenDoors:
				{
					switch ( m_eDoorState )
					{
						...
						{
							if( GetAnimationContext()->IsLocked() || m_bStartedToOpenDoor )
							{
								m_bStartedToOpenDoor = LTTRUE;
							}
							else if( !GetAnimationContext()->IsTransitioning() )
							{
...
void CAIHumanStrategyGrenadeThrow::Update()
{
	...
	if ( eStateThrow == m_eState )
	{
		...
		LTVector vOffsetDir;
		vOffsetDir = GetAI()->GetPosition() - vTargetPos;
		vOffsetDir.y = 0.f;
		vOffsetDir.Normalize();
		vTargetPos += vOffsetDir * GetAI()->GetBrain()->GetAttackGrenadeThrowOffset(); 
...
void CAIHumanStrategyGrenadeThrow::Throw(LTFLOAT fTime /*= 0.5f*/)
{
	...
	GetAI()->PlaySound( kAIS_Grenade, LTTRUE );
}

LTBOOL CAIHumanStrategyGrenadeThrow::ShouldThrow( LTFLOAT fProbabilityFactor )
{
	if ( eGrenadeStatusDontThrow == GetAI()->GetBrain()->GetGrenadeStatus( fProbabilityFactor ) )
	{
		return LTFALSE;
	}
...
void CAIHumanStrategyShoot::Fire()
{
	...
	CWeapon* pWeapon = GetAI()->GetCurrentWeapon();
	if ( pWeapon && (pWeapon->GetWeaponData()->nAIWeaponType == kAIWeap_Melee) )
	{
