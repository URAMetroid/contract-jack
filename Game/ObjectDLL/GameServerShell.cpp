...
LTRESULT CGameServerShell::OnServerInitialized()
{
	...
	// Set our default gravity.  Also set on client.
	g_pPhysicsLT->SetGlobalForce( LTVector( 0.0f, DEFAULT_WORLD_GRAVITY, 0.0f ));
	...
	if( !InitGameType( **ppServerGameOptions ))
		return LT_ERROR;

	// Default to failing to initialize.
	( **ppServerGameOptions ).m_eServerStartResult = eServerStartResult_Failed;
...
void CGameServerShell::OnAddClient(HCLIENT hClient)
{
	...
	if( !BanIPMgr::Instance( ).OnAddClient( hClient ))
	{
		...
		cMsg.Writeuint8((uint8)MID_HANDSHAKE_BANNED);
	...
	if( IsMultiplayerGame( ))
	{
		...
		ModelId ePlayerModelId = (ModelId)ncd.m_ePlayerModelId;
		...
		GenerateUniqueName( hClient, ncd.m_szName, szUniqueName, ARRAY_LEN( szUniqueName ));
	...
	if( IsMultiplayerGame( ))
	{
		pClientData->m_eClientConnectionState = eClientConnectionState_Handshaking;
	...
	{
		// Consider client good and let him finish connecting.
		pClientData->m_eClientConnectionState = eClientConnectionState_Connected;
		g_pLTServer->FinishClientConnect( pClientData->m_hClient );
	}
...
void CGameServerShell::OnRemoveClient(HCLIENT hClient)
{
	...
	{
        uint32 nClientId = g_pLTServer->GetClientID( hClient );
		m_pGameSpyServer->ReleaseCDKey( nClientId );
	}
...
LPBASECLASS	CGameServerShell::OnClientEnterWorld(HCLIENT hClient)
{
	...
	if (IsMultiplayerGame( ))
	{
		...
		ClientData* pClientData = GetClientData( hClient );
		if( !pClientData )
		{
		...
		cMsg.WriteString( pClientData->m_sUniqueName.c_str( ));
		cMsg.Writeuint32( nClientID );
		g_pLTServer->SendToClient( cMsg.Read(), hClient, MESSAGE_GUARANTEED );
	...
	for ( HCLIENTREF hCurClient = g_pLTServer->GetNextClientRef( LTNULL ); 
		  hCurClient; 
...
void CGameServerShell::ProcessHandshake(HCLIENT hClient, ILTMessage_Read *pMsg)
{
	...
	// Tell the client it's waiting for authorization.
	ClientData* pClientData = GetClientData( hClient );
	if( !pClientData )
	{
	...
	switch (nHandshakeSub)
	{
		case MID_HANDSHAKE_HELLO :
		{
			...
			bool bDoCDKey = ( m_pGameSpyServer != NULL );
			...
			if( bDoCDKey )
			{
				...
				uint32 nSizeChallenge = ARRAY_LEN( szChallenge );
				m_pGameSpyServer->GetCDKeyChallenge( szChallenge, nSizeChallenge );
		...
		case MID_HANDSHAKE_LETMEIN :
		{
			...
			static uint32 nWeaponCRC = g_pWeaponMgr->GetFileCRC();
			uint32 nMaskedWeaponCRC = nWeaponCRC ^ nXORMask;
			uint32 nClientCRC = pMsg->Readuint32();
			if (nMaskedWeaponCRC != nClientCRC)
			{
				bInvalidAssets = true;
			}
			...
			static uint32 nModelButesCRC = g_pModelButeMgr->GetFileCRC( );
			...
			uint32 nClientAutoexecCRC = pMsg->Readuint32();
			if( nClientAutoexecCRC !=  nAutoexecMaskedCRC )
			{
				bInvalidAssets = true;
			}
			...
			if( bInvalidAssets )
			{
				// Tell the client so it can disconnect itself.
				CAutoMessage cMsg;
				cMsg.Writeuint8(MID_HANDSHAKE);
				cMsg.Writeuint8((uint8)MID_HANDSHAKE_INVALIDASSETS);
				g_pLTServer->SendToClient(cMsg.Read(), hClient, MESSAGE_GUARANTEED);
				ClientSelfBoot( *pClientData );
				return;
			}
			...
			{
				char szChallenge[256];
				pMsg->ReadString( szChallenge, ARRAY_LEN( szChallenge ));
				char szChallengeResponse[256];
				pMsg->ReadString( szChallengeResponse, ARRAY_LEN( szChallengeResponse ));
				uint32 nClientId = g_pLTServer->GetClientID( hClient );
				uint8 aTcpIp[4];
				uint16 nPort;
				g_pLTServer->GetClientAddr( hClient, aTcpIp, &nPort );
				char szIpAddr[16];
				sprintf( szIpAddr, "%d.%d.%d.%d", aTcpIp[0], aTcpIp[1], aTcpIp[2], aTcpIp[3] );
				m_pGameSpyServer->ProcessCDKeyChallengeResponse( nClientId, szIpAddr, szChallenge, szChallengeResponse );
			}

			if( bDoCDKey )
			{
				pClientData->m_eClientConnectionState = eClientConnectionState_WaitingForAuth;
			}
			else
			{
				pClientData->m_eClientConnectionState = eClientConnectionState_Connected;
	...
	if( nCount > 1 )
	{
		...
		switch( GetGameType() )
		{
			...
			case eGameTypeDoomsDay:
			{
				...
				{
					if (pPlayer)
						DebugCPrint(1, "No Team start point for team (%i)!", pPlayer->GetTeamID() );
...
void CGameServerShell::UpdateSwitchingWorlds( )
{
	switch( m_eSwitchingWorldsState )
	{
		...
		case eSwitchingWorldsStatePlayerHookup:
		{
			...
			// up all the clients to players before moving on.
			...
			{
				// No more unclaimed clientrefs.  Now do the reloadsave.
				if( !g_pServerSaveLoadMgr->CanSaveGame( ) )
				{
					// Can't save, so skip the reloadsave state.
					SetSwitchingWorldsState( eSwitchingWorldsStateWaitForClient );
					return;
				}
		...
		case eSwitchingWorldsStateWaitForClient:
		{
			...
			for( ; iter != m_ClientDataList.end( ); iter++ )
			{
				...
				if( ClientConnectionInTrouble( pClientData->m_hClient ))
				{
					// Kick the client if they aren't communicating.
				...
				nNumClients++;
				if( pClientData->m_nLastTeamId < MAX_TEAMS )
				{
					nNumClientsOnTeam[pClientData->m_nLastTeamId]++;
				}

				if( !pClientData->m_hClient )
					continue;
				CPlayerObj* pPlayerObj = ( CPlayerObj* )g_pLTServer->GetClientUserData(pClientData->m_hClient);
				if( !pPlayerObj || !pPlayerObj->IsClientLoaded( ))
					continue;
				...
				if( pClientData->m_nLastTeamId < MAX_TEAMS )
				{
					nNumClientsOnTeamReady[pClientData->m_nLastTeamId]++;
				}
			}
			...
			if( IsTeamGameType( ))
			{
				...
				{
					// If their are people on this team, but no one's ready, we need to wait.
					if( nNumClientsOnTeam[i] != 0 && nNumClientsOnTeamReady[i] == 0 )
					{
						return;
					}
				}
			...
			// On non-team games, just make sure there is at least one person ready to play.
			else
			{
...
bool CGameServerShell::InitGameType( ServerGameOptions const& serverGameOptions )
{
	m_ServerGameOptions = serverGameOptions;
	...
	switch( m_ServerGameOptions.m_eGameType )
	{
		...
		case eGameTypeTeamDeathmatch:
			...
			break;
		case eGameTypeDemolition:
			m_pServerMissionMgr = debug_new( CDemolitionMissionMgr );
			break;
		case eGameTypeDoomsDay:
...
void CGameServerShell::UpdateMultiplayer()
{
	...
	// Update any clients waiting to get in.
	UpdateClientsWaitingForAuth( );
	...
	// Remove any clients that have violated rules.
	KickBadClients( );
...
bool CGameServerShell::UpdateClientsWaitingForAuth( )
{
	ClientDataList::iterator iter = m_ClientDataList.begin( );
	for( ; iter != m_ClientDataList.end( ); iter++ )
	{
		...
		// Check if they are waiting.
		if( pClientData->m_eClientConnectionState != eClientConnectionState_WaitingForAuth )
			continue;

		// Check if we're using gamespy.
		if( m_pGameSpyServer )
		{
			uint32 nClientId = g_pLTServer->GetClientID( pClientData->m_hClient );
			IGameSpyServer::EChallengeResponse eChallengeResponse = m_pGameSpyServer->GetChallengeResponse( nClientId );
			switch( eChallengeResponse )
			{
				...
				case IGameSpyServer::eChallengeResponse_Accepted:
				{
					...
					g_pLTServer->SendToClient(cMsg.Read(), pClientData->m_hClient, MESSAGE_GUARANTEED);
				...
				case IGameSpyServer::eChallengeResponse_Rejected:
				{
					...
					CAutoMessage cMsg;
					cMsg.Writeuint8(MID_HANDSHAKE);
					cMsg.Writeuint8((uint8)MID_HANDSHAKE_BADCDKEY);
					g_pLTServer->SendToClient(cMsg.Read(), pClientData->m_hClient, MESSAGE_GUARANTEED);
					ClientSelfBoot( *pClientData );
					break;
				}
...
bool CGameServerShell::KickBadClients( )
{
	...
	uint32 nAutoBootTime = ( uint32 )( GetConsoleFloat( "AutoBootTime", 1.0f * 60.0f ) * 1000.0f + 0.5f );
	...
	for( ; iter != m_ClientDataList.end( ); iter++ )
	{
		...
		if( !pClientData )
		{
			ASSERT( !"CGameServerShell::KickBadClients:  Invalid clientdata object." );
			continue;
		}

		// Check if they are supposed to be kicked.
		if( pClientData->m_bWasKicked )
		{
			g_pLTServer->CPrint( "Removing kicked client." );
			g_pLTServer->KickClient( pClientData->m_hClient );
			continue;
		}
...
bool CGameServerShell::UpdateGameSpy( )
{
		...
		((nCurTime - m_nLastPublishTime) > k_nRepublishDelay))
	{
		...
		sprintf( szValue, "FriendlyFire %d; WeaponsStay %d; Rounds %d; RunSpeed %d", sms.m_bFriendlyFire,
			sms.m_bWeaponsStay, sms.m_nRounds, sms.m_nRunSpeed );
...
CPlayerObj* CGameServerShell::CreatePlayer(HCLIENT hClient, ModelId ePlayerModelId )
{
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CGameServerShell::IsCapNumberOfBodies()
//
//	PURPOSE:	Cap the number of bodies in a radius.
//
// ----------------------------------------------------------------------- //

bool CGameServerShell::IsCapNumberOfBodies( )
{
	switch( GetGameType( ))
	{
		case eGameTypeCooperative:
...
// ----------------------------------------------------------------------- //
//
//  ROUTINE:	CPlayerObj::IsNameTaken
//
//  PURPOSE:	Checks if any client has the given name.
//
// ----------------------------------------------------------------------- //
bool CGameServerShell::IsNameTaken( char const* pszName, HCLIENT hSkipClient ) const
{
	// Remove this client from our list of clients.
	ClientDataList::const_iterator iter = m_ClientDataList.begin( );
	for( ; iter != m_ClientDataList.end( ); iter++ )
	{
		ClientData const* pClientData = *iter;
		if( !pClientData )
			continue;

		if( pClientData->m_hClient == hSkipClient )
			continue;

		// Check if our name is different than this player
		if( stricmp( pszName, pClientData->m_sUniqueName.c_str( )) == 0 )
			return true;
	}
...
//
// ----------------------------------------------------------------------- //
void CGameServerShell::GenerateUniqueName( HCLIENT hClient, char const* pszNameBase, char* pszUniqueName, int nUniqueNameSize ) const
{
	char szUniqueName[256];
	strncpy( szUniqueName, pszNameBase, ARRAY_LEN( szUniqueName ));
	szUniqueName[ARRAY_LEN( szUniqueName ) - 1] = 0;
	...
	if( IsNameTaken( szUniqueName, hClient ))
	{
		...
		while( 1 )
		{
			_snprintf( szUniqueName, ARRAY_LEN( szUniqueName ), "%s%d", pszNameBase, nCount );
			szUniqueName[ARRAY_LEN( szUniqueName ) - 1] = 0;
			if( !IsNameTaken( szUniqueName, hClient ))
				break;
	...
	// Copy the string.
	strncpy( pszUniqueName, szUniqueName, nUniqueNameSize );
...
//  ROUTINE:	CPlayerObj::GenerateUniqueName
//
//  PURPOSE:	Client told to self boot.  Flag him for removal later.
//
// ----------------------------------------------------------------------- //
void CGameServerShell::ClientSelfBoot( ClientData& clientData )
{
