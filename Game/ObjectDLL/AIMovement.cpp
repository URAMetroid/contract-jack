...
LTBOOL CAIMovement::Update()
{
	...
	if( bMove && ( m_pAI->GetPosition() != vNewPos ) )
	{
		...
		if( ( pVolume && pVolume->IsVolumeEnabled() ) || m_bIgnoreVolumes || ( m_eState == eStateSet ) )
		{
			if( m_eState != eStateSet )
			{
	...
	else
	{
		...
		if( eMovementType == kAM_Encode_V )
		{
			...
			*pvNewPos = vOldPos + ( vOldDirToDest * vHintPosDelta.Length() );
			...
			if( vOldDirToDest.Mag() == 0.f )
			{
				AIError( "AI TRYING TO NORMALIZE A ZERO LENGTH VECTOR!" );
				Clear();
				m_eState = eStateDone;
				*pvNewPos = m_vDest;
				return LTTRUE;
