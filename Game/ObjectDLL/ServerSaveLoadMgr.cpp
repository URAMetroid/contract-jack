...
bool CServerSaveLoadMgr::LoadCheckpointSave( )
{
	...
	// Load the save file.
	if( !LoadSaveFile( CHECKPOINTSAVE_INIKEY, GetCheckpointSaveDir( ), GetCheckpointSaveFile( )))
		return false;
...
bool CServerSaveLoadMgr::SaveCheckpointSave( int nMission, int nLevel )
{
	...
	m_sSaveGameDir = GetCheckpointSaveDir( );
	m_sSaveGameFile = GetCheckpointSaveFile( );
...
bool CServerSaveLoadMgr::FinishSaveGame( )
{
	...
	if( bRet )
	{
		...
		if( stricmp( m_sSaveGameKey, RELOADLEVEL_INIKEY ) == 0 )
		{
			DeleteFile( GetCheckpointSaveFile( ));
			CWinUtil::RemoveDir( GetCheckpointSaveDir( ));
			WriteSaveINI( CHECKPOINTSAVE_INIKEY, "", "" );
		}
	}
