...
// -------------------------------------------------------------//
//
// Function name   : CLiteObjectMgr::RemoveObject
// Description     : Removes the object from game.
// Return type     : void 
// Argument        : GameBaseLite *pObject - the object to remove.
// Argument        : bool bImmediate - remove immediately, or queue 
//						for next update.
//
// -------------------------------------------------------------//
void CLiteObjectMgr::RemoveObject(GameBaseLite *pObject, bool bImmediate)
{
	...
	if (bRemoveResult)
	{
		// Check if to be removed immediately, or queued for later removal.  Object
		// may need to be queued if in the middle of a message handling function or update.
		if( bImmediate )
		{
		...
		{
			// No longer consider this object registered, so that calls on it will fail.
			pObject->SetRegistered( false );

			// Queue it for later removal during update.
			m_aDeleteObjects.push_back( pObject );
		}
	}
	else
	{
...
{
	// If not registered, then can't be in lists.
	if( !pObject->IsRegistered( ))
		return false;
...
{
	// Ignore if object not registered.
	if( !pObject->IsRegistered( ))
		return;
...
void CLiteObjectMgr::Update()
{
	...
	// See if any more objects need to be deleted.
	HandlePendingDeletes();
