...
uint32 PickupItem::EngineMessageFn(uint32 messageID, void *pData, LTFLOAT fData)
{
	switch(messageID)
	{
		...
		{
			if( !m_bTouchPickup || !m_bFullyCreated )
				break;
...
uint32 PickupItem::ObjectMessageFn(HOBJECT hSender, ILTMessage_Read *pMsg)
{
	...
	switch(messageID)
	{
		case MID_PICKEDUP:
		{
			...
			bool bDisplayMsg = pMsg->Readbool( );
			bool bWeaponsStay = pMsg->Readbool( );
			PickedUp( bWasPickedUp, bDisplayMsg, bWeaponsStay );
		}
		break;
...
bool PickupItem::OnTrigger(HOBJECT hSender, const CParsedMsg &cMsg)
{
	...
	if( cMsg.GetArg(0) == s_cTok_Activate )
	{
		...
		if (IsPlayer(hSender))
		{
			...
			// Set the state that the player is activating a pickup at the 
			// the moment so we can send the right message to the client.
			// The client needs to know whether to show a "full of this item"
			// message.  Since there were so many paths to getting to the
			// client, this method was chosen to get the implementation done
			// quickly.
			pPlayer->SetActivatingPickup( true );

				ObjectTouch(hSender);

			pPlayer->SetActivatingPickup( false );
		}
		else
		{
			SetPlayerObj(LTNULL);
...
void PickupItem::PickedUp( bool bWasPickedUp, bool bDisplayMsg, bool bWeaponsStay )
{
	// 08/04/03 - KEF - If we're in multiplayer (so single player doesn't get broken), and we
	// already had whatever was provided, stop sending messages around.  This filters out a
	// lot of extra "picked up" messages for pickups that the player already has.
	if (IsMultiplayerGame() && !bWasPickedUp)
	{
		return;
	}
