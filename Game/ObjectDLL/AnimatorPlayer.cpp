...
#define _N eAniInvalid
    ...
    m_AniMotorcycle		            = AniPlayerMain            (LTTRUE,     _A("LRdM"),			_N,					_N,					_N,					_N);
...
void CAnimatorPlayer::ResetAniTracker(AniTracker eAniTracker)
{
	...
	// 08/05/03 - KEF - This no longer seems to be required, since this only resets the animation on the clients.
	// (which is something the networking handles for the players anyway.)  But just in case, make sure it still
