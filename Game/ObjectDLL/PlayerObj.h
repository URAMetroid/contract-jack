...
class CPlayerObj : public CCharacter
{
	public :
		...
		bool HandleSecretMessage();
		bool HandleChangeModelMessage(const CParsedMsg &cMsg);
		...
		// Accessor to activatingpickup state.
		bool	IsActivatingPickup( ) const { return m_bActivatingPickup; }
		void	SetActivatingPickup( bool bValue ) { m_bActivatingPickup = bValue; }
	...
	protected :
		...
		virtual bool	SetPhysicsModel( PlayerPhysicsModel eModel = PPM_NORMAL, uint8 nModelId = (uint8)-1, bool bUpdateClient = true );
		virtual bool	SetVehiclePhysicsModel( uint8 nModelId );
		...
		void	AcquireWeapon( const char* pWeaponName );
	...
	// should be added here (to keep them together)...
		...
		// If the player is on a vehicle with a weapon, this is the weapon.
		
		uint8		m_nVehicleWeaponId;

		// If the player is on a vehicle this is the id used to identify the vehicle.

		uint8		m_nVehicleId;

		// Indicates that the player is activating a pickup at this time.
		bool		m_bActivatingPickup;

		// Weapon sound looping status
		uint32		m_nWeaponSoundLoopType, m_nWeaponSoundLoopWeapon;

		typedef std::set<uint8> OverlaySet;
		OverlaySet	m_ActiveOverlays;
};
