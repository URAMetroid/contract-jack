...
bool KeyFramer::CreateObjectList()
{
	...
	if( !m_pKeys || !m_sObjectName.length( ))
		return false;
...
bool KeyFramer::Init()
{
	// Make keys if we don't have them.
	if( !m_pKeys )
		if( !CreateKeyList( ))
			return false;

	// Make objectlist if we don't have it.
	if( !m_pObjectList )
		if( !CreateObjectList( ))
			return false;
...
void KeyFramer::Update()
{
	...
	if (m_bFirstUpdate)
	{
		...
		if (m_bStartActive || m_bStartPaused)
		{
			...
			if( !m_pObjectList || m_pObjectList->m_nInList == 0 )
			{
				if( !CreateObjectList( ))
					return;
			}
...
void KeyFramer::Save(ILTMessage_Write *pMsg, uint32 dwSaveFlags)
{
    ...
    SAVE_CHARSTRING(m_sObjectName.c_str( ));
