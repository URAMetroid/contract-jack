...
void VolumeBrush::ReadProp(ObjectCreateStruct *pStruct)
{
	...
	{
		// We need to remain backwards compatible so look for the old PhysicsModel of 'Snowmobile'
		// and make sure it uses the new Vehicle model...

		if( genProp.m_String[0] )
		{
			if( !_stricmp( genProp.m_String, "Snowmobile" ))
			{
				m_ePPhysicsModel = PPM_VEHICLE;
			}
			else
			{
