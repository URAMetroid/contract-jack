...
void CAIGoalCheckBody::RecalcImportance()
{
	...
	if( ( m_lstSeenBodies.size() > 0 ) && 
		...
		( g_pLTServer->GetTime() > m_fDelayTime ) )
	{
...
{
	// Do not check any bodies for 30 seconds after seeing an enemy.
	// (It looks dumb when an enemy goes out of sight, and AI
	// immediately check a body.

	if( pSenseRecord->eSenseType == kSense_SeeEnemy )
	{
		m_fDelayTime = g_pLTServer->GetTime() + 30.f;
	}
	...
	if( super::HandleGoalSenseTrigger(pSenseRecord) )
	{
		...
		if( pSenseRecord->eSenseType == kSense_SeeAllyDeath )
		{
			...
			if( g_pLTServer->GetTime() > m_fDelayTime )
			{
				return LTTRUE;
			}
		}
