...
// ----------------------------------------------------------------------- //
//
//  ROUTINE:	Door::UpdatePowerOff
//
//  PURPOSE:	Handel updating the PowerOff state
//
// ----------------------------------------------------------------------- //

void Door::UpdatePowerOff( const LTFLOAT &fCurTime )
{
	// Do not allow anyone to close a door if AI are near by.
	// This is necessary because AI are non-solid, and will not 
	// stop the door themselves.
	...
	LTVector vPos;
	g_pLTServer->GetObjectPos( m_hObject, &vPos );
	if( g_pCharacterMgr->FindCharactersWithinRadius( &lstChars, vPos, 64.f, CCharacterMgr::kList_AIs ) )
	{
		pChar = *(lstChars.GetItem( TLIT_FIRST ));
		if( pChar )
		{
			m_hActivateObj = pChar->m_hObject;
			SetPowerOn();
			return;
		}
	}
	...
	ActiveWorldModel::UpdatePowerOff( fCurTime );
}
