...
/**
		[RP] 09/10/03 - Casting an arbitrary ray back at the object and using the surface of the
			poly it hit as the surface of the entire object has many problems.  There could be many 
			different surfaces of the world model and using just one doesn't look right.  There
			is also no quarentee that a poly was hit.  Just leave the object surface type as UNKNOWN
			so the surface of the polygon will be used instead.
