...
CMDMGR_BEGIN_REGISTER_CLASS( CCharacter )
	...
	CMDMGR_ADD_MSG( MOVETOFLOOR, 2, NULL, "MOVETOFLOOR <0 or 1>" )
...
bool CCharacter::OnTrigger(HOBJECT hSender, const CParsedMsg &cMsg)
{
	...
	static CParsedMsg::CToken s_cTok_MoveToFloor("MOVETOFLOOR");
	...
	else if ( cMsg.GetArg(0) == s_cTok_MoveToFloor )
	{
		if( cMsg.GetArgCount() > 1 )
		{
			SetMoveToFloor( IsTrueChar( *cMsg.GetArg(1) ) != 0 );
		}
...
void CCharacter::SetCarrying( uint8 nCarrying, ChassisPieceBute* pChassisPieceBute )
{
	if( m_nCarrying != nCarrying )
	{
		m_nCarrying = nCarrying;
		m_pChassisPieceBute = pChassisPieceBute;
		...
		cMsg.Writeuint8( pChassisPieceBute ? pChassisPieceBute->m_nIndex : CHASSISPIECEBUTE_INVALID );
		g_pLTServer->SendToClient( cMsg.Read(), LTNULL, MESSAGE_GUARANTEED );
...
void CCharacter::CreateSpecialFX(LTBOOL bUpdateClients /* =LTFALSE */)
{
	...
	m_pcs->pChassisPieceBute		= m_pChassisPieceBute;
...
void CCharacter::SetPitch( float fPitch )
{
	...
	if( IsMultiplayerGame() )
	{
		if( m_fLastPitch != m_fPitch )
		{
			...
			g_pLTServer->SetObjectUnguaranteedData( m_hObject, CompressAngleToByte( fPitch ) );
...
void CCharacter::SetupBody(Body* pProp)
{
	...
	bi.bCanRevive = false;
	bi.bMoveToFloor = m_bMoveToFloor;
