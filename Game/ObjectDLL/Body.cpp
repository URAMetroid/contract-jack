...
void Body::RemovePowerupObjects()
{
	for (uint32 iWeapon = 0; iWeapon < kMaxWeapons; iWeapon++)
	{
		if ( m_ahWeapons[iWeapon] )
		{
			...
			g_pCommonLT->SetObjectFlags( m_ahWeapons[iWeapon], OFT_Flags, 0, FLAG_TOUCH_NOTIFY );
...
void Body::Init(const BODYINITSTRUCT& bi)
{
	...
	m_bMoveToFloor = bi.bMoveToFloor;
	...
	// [RP] 9/09/03 - No searching in game so don't add it to the aggrgate...
/*	
	if (m_cWeapons)
	{
		bool bAdded = false;
...
*/
	...
	{
		// Don't allow carrying bodies at all.
		m_BCS.bCanBeCarried = m_bCanBeCarried = m_bCanCarry = false;
	}
...
uint32 Body::Update(void *pData, float fData)
{
	...
	// Update as a prop 
	uint32 dwRet = super::EngineMessageFn(MID_UPDATE, pData, fData);
	...
	if ( !bUpdatingDims && !m_bDimsUpdated && (!m_pState || m_pState->CanUpdateDims( )))
	{
		...
		// Allow de-activation, since that's what we really want
		m_bCanDeactivate = true;
	...
	// Allow other functions to cause deactivation
	m_bCanDeactivate = true;
...
void Body::AddWeapon(HOBJECT hWeapon, const char *pszPosition)
{
...
{
	// Don't allow any carrying of bodies.
	bCarry = false;
