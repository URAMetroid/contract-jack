...
static CVarTrack g_vtDebugEventCounter;
static const float kDebugTriggers  = 1.0f;
static const float kDebugFunctions = 2.0f;

// Debug only utility function...
static const char *GetEventCounterName(HOBJECT hObj)
{
...
uint32 EventCounter::EngineMessageFn(uint32 messageID, void *pData, LTFLOAT fData)
{
	switch(messageID)
	{
		case MID_PRECREATE:
		{
            if (!g_vtDebugEventCounter.IsInitted())
			{
				g_vtDebugEventCounter.Init(g_pLTServer, "EventCounterDebug", NULL, 0.0f);
			}
...
bool EventCounter::OnTrigger(HOBJECT hSender, const CParsedMsg &cMsg)
{
	...
	if (g_vtDebugEventCounter.GetFloat() >= kDebugTriggers)
	{
		g_pLTServer->CPrint("EventCounter::OnTrigger(): (%s)", GetEventCounterName(m_hObject));
		bShowDebugInfo = true;
	}
		...
		(cMsg.GetArg(0) == s_cTok_Increment))
	{
		...
		if (bShowDebugInfo)
		{
			g_pLTServer->CPrint("  INCREMENT: %d", nCount);
		}
			 ...
			 (cMsg.GetArg(0) == s_cTok_Decrement))
	{
		...
		if (bShowDebugInfo)
		{
			g_pLTServer->CPrint("  DECREMENT: %d", nCount);
		}
...
{
	bool bShowDebugInfo = false;
	if (g_vtDebugEventCounter.GetFloat() >= kDebugFunctions)
	{
		g_pLTServer->CPrint("EventCounter::Increment(): (%s) %s", 
			GetEventCounterName(m_hObject), m_bLocked ? "LOCKED" : "UNLOCKED");
		bShowDebugInfo = true;
	}
	...
	if (bShowDebugInfo)
	{
		g_pLTServer->CPrint("  m_nCurVal = %d", m_nCurVal); 
	}
		...
		if (m_EventData[i].nValue == m_nCurVal)
		{
			if (m_EventData[i].hstrIncToValCmd)
			{
				...
				if (pCmd && g_pCmdMgr->IsValidCmd(pCmd))
				{
					...
					if (bShowDebugInfo)
					{
						g_pLTServer->CPrint("  Processed: %s", pCmd); 
					}
				}
			}
		}
...
void EventCounter::Decrement()
{
	...
	if (g_vtDebugEventCounter.GetFloat() >= kDebugFunctions)
	{
		g_pLTServer->CPrint("EventCounter::Decrement(): (%s) %s", 
			GetEventCounterName(m_hObject), m_bLocked ? "LOCKED" : "UNLOCKED");
		bShowDebugInfo = true;
	}
