...
static const float s_fPlayerBodyLifetime = 15.0f;
...
//					Message		Num Params	Validation FnPtr		Syntax
	...
	CMDMGR_ADD_MSG( TRANSMISSION,	-1,		ValidateTransmissionMsg,"TRANSMISSION <transmission id> [ActivePlayerComm] [TeamNumber] [SoundVoiceId]" )
	...
	CMDMGR_ADD_MSG( SECRET,			1,		NULL,					"SECRET" )
	CMDMGR_ADD_MSG( CHANGEMODEL,	2,		NULL,					"ChangeModel <modelname>" )
...
//	ResetObjectRelationMgr();
	...
	m_nVehicleWeaponId = WMGR_INVALID_ID;
	m_nVehicleId = VBMGR_INVALID_ID;
	
	m_bActivatingPickup = false;
	...
	switch(messageID)
	{
		...
		case MID_PRECREATE:
		{
			if( fData != PRECREATE_SAVEGAME )
			{
				...
				if (pStruct)
				{
					pStruct->m_Flags2 |= FLAG2_PLAYERCOLLIDE | FLAG2_SPECIALNONSOLID;
...
bool CPlayerObj::OnTrigger(HOBJECT hSender, const CParsedMsg &cMsg)
{
	...
	static CParsedMsg::CToken s_cTok_Secret( "SECRET" );
	static CParsedMsg::CToken s_cTok_ChangeModel( "ChangeModel" );
	...
	else if( cMsg.GetArg(0) == s_cTok_Secret )
	{
		HandleSecretMessage();
		bRet = true;
	}
	else if( cMsg.GetArg(0) == s_cTok_ChangeModel )
	{
		HandleChangeModelMessage(cMsg);
		bRet = true;
	}
...
bool CPlayerObj::FilterDamage( DamageStruct *pDamageStruct )
{
	...
	// Change damage a little if riding a vehicle...

	if( m_ePPhysicsModel == PPM_VEHICLE )
	{
...
// --------------------------------------------------------------------------- //
//
//	ROUTINE:	CPlayerObj::HandleSecretMessage()
//
//	PURPOSE:	Process an secret message
//
// --------------------------------------------------------------------------- //

bool CPlayerObj::HandleSecretMessage( )
{

	GetMissionStats()->dwNumSecrets++;

	CAutoMessage cClientMsg;
	cClientMsg.Writeuint8( MID_PLAYER_INFOCHANGE );
	cClientMsg.Writeuint8( IC_SECRET );
	g_pLTServer->SendToClient(cClientMsg.Read(), GetClient( ), MESSAGE_GUARANTEED);
...
// --------------------------------------------------------------------------- //
//
//	ROUTINE:	CPlayerObj::HandleChangeModelMessage()
//
//	PURPOSE:	Process an ChangeModel message
//
// --------------------------------------------------------------------------- //

bool CPlayerObj::HandleChangeModelMessage( const CParsedMsg &cMsg )
{
	if (cMsg.GetArgCount() < 2) return false;

	ModelId eModelId = g_pModelButeMgr->GetModelId(cMsg.GetArg(1).c_str());
    if (eModelId == eModelIdInvalid)
	{
		g_pLTServer->CPrint("CPlayerObj::HandleChangeModelMessage() : %s is not a valid model name.",cMsg.GetArg(1).c_str());
		return false;
	}
	m_eModelId = eModelId;
	m_eModelSkeleton = g_pModelButeMgr->GetModelSkeleton(m_eModelId);
...
bool CPlayerObj::HandleOverlayMessage(const CParsedMsg &cMsg)
{
	...
	if(nMsg < 255)
	{
		...
		if (nMsg)
		{
			m_ActiveOverlays.insert(nId);
		}
		else
		{
			m_ActiveOverlays.erase(nId);
		}
...
void CPlayerObj::SaveOverlays(ILTMessage_Write *pMsg)
{
	...
	OverlaySet::iterator iter = m_ActiveOverlays.begin();
	while (iter != m_ActiveOverlays.end())
	{
		uint8 n = *iter;
		pMsg->Writeuint8(*iter);
		iter++;
	}
}
...
#pragma MESSAGE( "TODO - Add an 'AnimationType' property to the VehicleButes..." )

		if( m_ePPhysicsModel == PPM_VEHICLE )
		{
			VEHICLE *pVehicleData = g_pVehicleButeMgr->GetVehicle( m_nVehicleId );
			if( pVehicleData )
			{
				m_Animator.SetMain( pVehicleData->bMotorcycle ? CAnimatorPlayer::eMotorcycle : CAnimatorPlayer::eSnowmobile);
			}
		}
...
void CPlayerObj::WriteVehicleMessage(ILTMessage_Write *pMsg)
{
	...
	else if( m_ePPhysicsModel == PPM_VEHICLE )
	{
		pMsg->Writeuint8( m_nVehicleId );
	}
	...
	cMsg.Writeuint8( CFX_PLAYER_PHYSICS_MODEL );
	cMsg.Writeuint8( m_ePPhysicsModel );
	cMsg.Writeuint8( m_nVehicleId );
...
void CPlayerObj::FullGearCheat()
{
    ...
    for (uint8 i=0; i < nNumGearTypes; i++)
	{
		...
		cMsg.Writeuint8(i);
		cMsg.Writebool( IsActivatingPickup( ));
		g_pLTServer->SendToClient(cMsg.Read(), GetClient( ), MESSAGE_GUARANTEED);
	}
}
...
#pragma MESSAGE( "TODO - Get the VehicleId from the GameStartPoint..." )
...
void CPlayerObj::AcquireDefaultWeapon()
{
	...
	// Give us the holster weapon.
	pWeaponData = g_pWeaponMgr->GetHolsterWeapon( );
	if( pWeaponData )
	{
		m_pPlayerAttachments->ObtainWeapon( pWeaponData->nId, AMMO_DEFAULT_ID, -1, LTTRUE );
	}
}
...
void CPlayerObj::AcquireWeapon( const char* pWeaponName )
{
	...
	if (pData->m_hTarget)
	{
		...
		switch (pData->m_nType)
		{
			...
			case MID_ACTIVATE_MOVE:
			{
				...
				else if( IsKindOf( pData->m_hTarget, "ChassisPiece" ) )
				{
...
LTBOOL CPlayerObj::MultiplayerInit(ILTMessage_Read *pMsg)
{
	...
	if( IsTeamGameType( ))
	{
		...
		ClientData* pClientData = g_pGameServerShell->GetClientData( GetClient( ));
		if( pClientData )
		{
			pClientData->m_nLastTeamId = nTeam;
		}
...
void CPlayerObj::Save(ILTMessage_Write *pMsg, uint32 dwSaveFlags)
{
	...
	// Save the ID to our vehicle weapon...

	SAVE_BYTE( m_nVehicleWeaponId );
	SAVE_BYTE( m_nVehicleId );
...
void CPlayerObj::UpdateSpecialFX()
{
	...
	// are *really* ducked...
	// 08/05/03 - KEF - Theoretically, this flag is not being used, and removing it seems to be safe
	// in multiplayer.  But just to be on the safe side, it's still set in single player.
	if ( !IsMultiplayerGame() && (m_Animator.GetLastMovement() == CAnimatorPlayer::eCrouching ))
	{
...
void CPlayerObj::HandleWeaponFireMessage(ILTMessage_Read *pMsg)
{
	...
	// If on a vehilce use the vehicle weapon if it has one...

	if( IsVehicleModel( m_ePPhysicsModel ) && g_pWeaponMgr->IsValidWeaponId( m_nVehicleWeaponId ))
	{
		pWeapon = m_pPlayerAttachments->GetWeapon( m_nVehicleWeaponId );
	}
...
void CPlayerObj::HandleClientMsg(ILTMessage_Read *pMsg)
{
    ...
    switch (byMessage)
	{
		...
		case CP_PHYSICSMODEL :
		{
			...
			SetPhysicsModel( eModel, nModelId, false );
		}
		break;
...
void CPlayerObj::RideVehicle( PlayerVehicle* pVehicle )
{
	...
	{
		g_pLTServer->CPrint("Can't ride vehicles while ducking");
		return;
	}
	...
	if( !SetPhysicsModel( PPM_VEHICLE, pVehicle->GetVehicleId(), LTTRUE ))
	{
		// Something went wrong so don't get on the vehicle...
...
void CPlayerObj::TransferWeapons(Body *pBody, bool bRemove )
{
	...
	if (m_pPlayerAttachments)
	{
		...
		if (pWeapon && pWeapon->Have())
		{
			...
			AMMO const *pAmmo     = pWeapon->GetAmmoData();
			...
			{
				sprintf(szSpawn, "WeaponItem Gravity 0;MoveToFloor 0;AmmoAmount %d;WeaponType %s;AmmoType %s;MPRespawn 0, DMTouchPickup 0",
					pWeapon->GetAmmoInClip(), pWeaponStruct->szName, pAmmo->szName);

				LTVector vMyPos;
				g_pLTServer->GetObjectPos(m_hObject, &vMyPos);
				BaseClass* pObj = SpawnObject(szSpawn, vMyPos, LTRotation());

				// If we succeeded in creating a WeaponItem, hide it and add it to the body...
				if ( pObj && pObj->m_hObject )
				{
					pBody->AddWeapon(pObj->m_hObject, m_pPlayerAttachments->GetRightHand().GetName());
				}
			}
...
bool CPlayerObj::SetPhysicsModel( PlayerPhysicsModel eModel /* = PPM_NORMAL */, 
								  uint8 nModelId /* = VBMGR_INVALID_ID */,
								  bool bUpdateClient /* = true  */ )
{
	...
	{
		case PPM_VEHICLE :
			if( !SetVehiclePhysicsModel( nModelId ))
				return false;
		break;
...
bool CPlayerObj::SetVehiclePhysicsModel( uint8 nModelId )
{
	if( !g_pVehicleButeMgr->IsValidVehicleId( nModelId ))
		return false;
	...
	if (!m_hVehicleModel)
	{
		...
		sprintf(buff, "PlayerVehicle VehicleType %s;Gravity 1", pVehicle->sName.c_str());
	...
	// Set the vehicle weapon if the vehicle has one...
	...
	const char* pSocket = pVehicle->bMotorcycle ? "Motorcycle" : "Snowmobile";
	...
	if( !pVehicle->sVehicleWeapon.empty() )
	{
		const WEAPON *pWeapon = g_pWeaponMgr->GetWeapon( pVehicle->sVehicleWeapon.c_str() );
		if( pWeapon )
		{
			m_nVehicleWeaponId = pWeapon->nId;
			m_pPlayerAttachments->ObtainWeapon( m_nVehicleWeaponId );
		}
	}
...
void CPlayerObj::SetNormalPhysicsModel()
{
	...
	// We have no vehicle weapon anymore...
...
void CPlayerObj::SetCarriedObject( HOBJECT hObject, bool bTransition /* = false  */ )
{
	...
	if (m_hCarriedObject && !hObject)
	{
		...
		m_bCanDropBody = false;
		m_bCarryingAI = false;
		SetCarrying(CFX_CARRY_NONE, NULL);
	...
	if (hObject && !m_hCarriedObject)
	{
		...
		{
			SetCarrying(CFX_CARRY_BODY, NULL);
		}
		else
		{
			ChassisPiece* pChassisPiece = dynamic_cast< ChassisPiece* >( g_pLTServer->HandleToObject( hObject ));
			//we only know about carrying doomsday pieces right now
			if( pChassisPiece && pChassisPiece->GetChassisPieceBute( ))
			{
				SetCarrying(CFX_CARRY_CHASSIS_PIECE, const_cast< ChassisPieceBute* >( pChassisPiece->GetChassisPieceBute( )));
			}
			else
			{
				ASSERT( false );
...
bool CPlayerObj::IsCarryingHeavyObject() 
{
    ...
    if( m_pcs->nCarrying == CFX_CARRY_BODY )
		return true;
	
	if( m_pcs->nCarrying == CFX_CARRY_CHASSIS_PIECE )
	{
		ChassisPiece* pChassisPiece = dynamic_cast< ChassisPiece* >( g_pLTServer->HandleToObject( m_hCarriedObject ));
		//we only know about carrying doomsday pieces right now
		if( pChassisPiece && pChassisPiece->GetChassisPieceBute( ))
		{
			return pChassisPiece->GetChassisPieceBute( )->m_bHeavy;
		}
	}
...
bool CPlayerObj::DropCarriedObject( bool bTransition /* = false  */ )
{
	...
	else if( IsKindOf( m_hCarriedObject, "ChassisPiece" ))
	{
...
bool CPlayerObj::PickupObject( HOBJECT hObject )
{
	...
	else if( IsKindOf( hObject, "ChassisPiece" ))
	{
...
END HACK - for DM testing */
	...
	if (nLevelId == 0)
	{
		WEAPON const *pWeaponData = g_pWeaponMgr->GetWeapon( pMission->nSelectedWeapon );
		if( pWeaponData && !pWeaponData->bServerRestricted )
		...
		else
		{
			...
			if( pMission->bResetPlayer || g_pGameServerShell->GetLGFlags( ) == LOAD_NEW_GAME )
			{
				...
				if( pWeaponData )
				{
					ChangeWeapon( g_pWeaponMgr->GetCommandId(pWeaponData->nId), LTTRUE );
				}
			}
		}
	}
...
void CPlayerObj::SetNetClientData( NetClientData const& netClientData )
{
	...
	g_pGameServerShell->GenerateUniqueName( GetClient( ), netClientData.m_szName, szUniqueName, 
		ARRAY_LEN( szUniqueName ));
	ClientData* pClientData = g_pGameServerShell->GetClientData( GetClient( ));
	if( !pClientData )
		return;
	pClientData->m_sUniqueName = szUniqueName;
...
char const* CPlayerObj::GetNetUniqueName( )
{
	if( !GetClient( ))
	{
		ASSERT( !"CPlayerObj::GetNetUniqueName: Invalid client." );
		return "";
	}
	...
	return pClientData->m_sUniqueName.c_str( );
}
...
void CPlayerObj::SetupBody( Body* pBody )
{
	...
	float fLifetime = s_fPlayerBodyLifetime;
...
void CPlayerObj::CreateRespawnBody( )
{
	...
	//fade respawn bodys quickly
	pBody->ResetLifetime( 0.1f );
