...
bool AmmoBox::ReadProp(ObjectCreateStruct *pStruct)
{
	...
	if( g_pLTServer->GetPropGeneric( "Filename", &genProp ) == LT_OK && genProp.m_String[0] )
	{
		m_sOriginalFilename = genProp.m_String;
	}

	if( g_pLTServer->GetPropGeneric( "Skin", &genProp ) == LT_OK && genProp.m_String[0] )
	{
		m_sOriginalSkin = genProp.m_String;
	}
...
void AmmoBox::Save(ILTMessage_Write *pMsg, uint32 dwSaveFlags)
{
	...
	SAVE_CHARSTRING( m_sOriginalFilename.c_str( ));
	SAVE_CHARSTRING( m_sOriginalSkin.c_str( ));
}
...
void AmmoBox::Load(ILTMessage_Read *pMsg, uint32 dwLoadFlags)
{
	...
	char szString[_MAX_PATH*2];
	LOAD_CHARSTRING( szString, ARRAY_LEN( szString ));
	m_sOriginalFilename = szString;
	LOAD_CHARSTRING( szString, ARRAY_LEN( szString ));
	m_sOriginalSkin = szString;
}
...
void AmmoBox::PickedUp( bool bWasPickedUp, bool bDisplayMsg, bool bWeaponsStay )
{
	...
	PickupItem::PickedUp( bWasPickedUp, bDisplayMsg, bWeaponsStay );
...
void AmmoBox::Respawn( )
{
	...
	LTStrCpy( ocs.m_Filename, m_sOriginalFilename.c_str( ), ARRAY_LEN( ocs.m_Filename ));
	LTStrCpy( ocs.m_SkinName, m_sOriginalSkin.c_str( ), ARRAY_LEN( ocs.m_SkinName ));	
	LTStrCpy( ocs.m_RenderStyleName, s_szDefaultRS, ARRAY_LEN( ocs.m_RenderStyleName ));
