...
void CPlayerAttachments::SetWeaponAttachmentHiddenState( CAttachment& AttachmentWeapon )
{
	...
	bool bHidden = m_bHidden || ( pWeapon ? pWeapon->IsHidden( ) : false );
	g_pCommonLT->SetObjectFlags( AttachmentWeapon.GetModel( ), OFT_Flags, bHidden ? FLAG_FORCECLIENTUPDATE : ( FLAG_VISIBLE | FLAG_FORCECLIENTUPDATE ), FLAG_FORCECLIENTUPDATE | FLAG_VISIBLE );
}

// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CPlayerAttachments::HideAttachments
//
//	PURPOSE:	Hide/Show attachments.
//
// ----------------------------------------------------------------------- //

void CPlayerAttachments::HideAttachments(LTBOOL bHide)
{
	CHumanAttachments::HideAttachments( bHide );

	// Have the weapon attachment do it's own hiding.
...
void CAttachmentWeapon::CreateSpawnString(char* szSpawn)
{
	...
	if (pAmmo && pWeapon && pWeapon->szHHModel[0])
	{
		...
		sprintf(szSpawn, "WeaponItem Gravity 0;MoveToFloor 0;AmmoAmount %d;WeaponType %s;AmmoType %s;MPRespawn 0",
