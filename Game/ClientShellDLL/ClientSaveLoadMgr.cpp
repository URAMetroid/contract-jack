...
bool CClientSaveLoadMgr::HandleSaveData( ILTMessage_Read& msg )
{
	...
	// Don't show reloadsave's.
	if( g_pClientMultiplayerMgr->IsConnectedToRemoteServer( ) && eSaveDataState != eSaveDataReloadSave )
	{
		g_pGameClientShell->CSPrint( LoadTempString( IDS_SAVINGGAME ));
	}
	else if( eSaveDataState == eSaveDataCheckpointSave )
	{
		g_pGameClientShell->CSPrint( LoadTempString( IDS_SAVINGCHECKPOINT ));
	}
	...
	// If we're just doing a keepalive save, then we don't have any failure conditions.
	bool bCanSave = false;
