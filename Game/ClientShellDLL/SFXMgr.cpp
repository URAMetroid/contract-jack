...
CBankedList<ChassisPieceFX> g_SFXBank_ChassisPiece;
CBankedList<ChassisFX> g_SFXBank_Chassis;
...
void CSFXMgr::HandleSFXMsg(HLOCALOBJ hObj, ILTMessage_Read *pMsg)
{
	...
	switch (nId)
	{
		...
		case SFX_AIMMAGNET_ID :
		{
			...
			CreateSFX(nId, &AimMagnet);
		}
		break;

		case SFX_CHASSISPIECE_ID :
		{
			CHASSISPIECECREATESTRUCT ChassisPieceCS;

			ChassisPieceCS.hServerObj = hObj;
			uint8 nIndex = pMsg->Readuint8();
			if( nIndex != CHASSISPIECEBUTE_INVALID )
				ChassisPieceCS.pChassisPieceBute = ChassisButeMgr::Instance( ).GetChassisPieceButeList( )[nIndex];
			else
				ChassisPieceCS.pChassisPieceBute = NULL;
			ChassisPieceCS.bCarried = pMsg->Readbool();
			ChassisPieceCS.nTeam = pMsg->Readuint8();
			ChassisPieceCS.bPlanted = pMsg->Readbool();
			ChassisPieceCS.nActivateId = pMsg->Readuint8( );
			ChassisPieceCS.bBlinking = pMsg->Readbool( );

			CreateSFX(nId, &ChassisPieceCS);
		}
		break;

		case SFX_CHASSIS_ID :
		{
			CHASSISCREATESTRUCT ChassisCS;

			ChassisCS.hServerObj = hObj;
			ChassisCS.nTeam = pMsg->Readuint8();
			ChassisCS.fDropZoneRadius = pMsg->Readfloat();
			ChassisCS.nActivateId = pMsg->Readuint8( );

			CreateSFX(nId, &ChassisCS);
		}
		break;
...
void CSFXMgr::DeleteSFX(CSpecialFX* pFX)
{
	...
	switch(pFX->GetSFXID())
	{
		...
		case SFX_CHASSISPIECE_ID :
		{
			g_SFXBank_ChassisPiece.Delete((ChassisPieceFX*)pFX);
		}
		break;

		case SFX_CHASSIS_ID :
		{
			g_SFXBank_Chassis.Delete((ChassisFX*)pFX);
		}
		break;
							   ...
							   ILTMessage_Read *pMsg, HOBJECT hServerObj)
{
	...
	switch (nId)
	{
		...
		case SFX_CHASSISPIECE_ID :
		{
			pSFX = g_SFXBank_ChassisPiece.New();
		}
		break;

		case SFX_CHASSIS_ID :
		{
			pSFX = g_SFXBank_Chassis.New();
		}
		break;
...
void CSFXMgr::UpdateDynamicSpecialFX()
{
	...
	for (int j=0; j < DYN_ARRAY_SIZE; j++)
	{
		...
		for (int i=0; i < nNumSFX; i++)
		{
			...
			if( pSFX )
			{
				if (fTime >= pSFX->m_fNextUpdateTime)
				{
					if (!pSFX->Update())
					{
					...
					{
						pSFX->m_fNextUpdateTime = fTime + pSFX->GetUpdateDelta();
					}
				}
			}
		}
