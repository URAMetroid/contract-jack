...
namespace
{
	...
	float		s_fRecoilDecaySpeed = 0.0f;
}
...
//	InitTargetMgr();
	...
	m_nPlayerInfoLastCompPitch		= 0;
	m_nPlayerInfoLastCompRot		= 0;
...
void CPlayerMgr::Save(ILTMessage_Write *pMsg, SaveDataState eSaveDataState)
{
	...
	if( m_nCarryingObject == CFX_CARRY_CHASSIS_PIECE )
	{
		if( m_pCarryChassisPieceBute )
		{
			pMsg->WriteString( m_pCarryChassisPieceBute->m_sName.c_str( ));
		}
		else
		{
			pMsg->WriteString( "" );
...
void CPlayerMgr::Load(ILTMessage_Read *pMsg, SaveDataState eLoadDataState)
{
	...
	char szChassisPieceBute[256];
	pMsg->ReadString( szChassisPieceBute, ARRAY_LEN( szChassisPieceBute ));
	SetCarryingObject( nCarryingObject, ChassisButeMgr::Instance( ).GetChassisPieceBute( szChassisPieceBute ), false );
...
void CPlayerMgr::HandleMsgPlayerOrientation (ILTMessage_Read *pMsg)
{
	...
	m_vServerRot = pMsg->ReadLTVector();

	switch( nID )
	{
		case MID_ORIENTATION_ALL :
		{
			m_fPitch	= m_vServerRot.x;
			m_fYaw		= m_vServerRot.y;
			m_fRoll		= m_vServerRot.z;
		}
		break;
...
void CPlayerMgr::UpdateCamera()
{
	...
	if(!g_pGameClientShell->IsServerPaused() || !m_bCameraPosInited)
	{
		// Update the sway...
		...
		((ILTClientSoundMgr*)g_pLTClient->SoundMgr())->SetListener( LTFALSE, &vPos, &rRot, LTTRUE );
	}
	// Remember to continue updating the server if we're in multiplayer
	else if (IsMultiplayerGame() && g_pGameClientShell->IsServerPaused())
	{
...
void CPlayerMgr::UpdateWeaponModel()
{
	...
	// If we are on a vehicle update the vehicle weapon...

	if( m_pMoveMgr->GetVehicleMgr()->IsVehiclePhysics() && m_pMoveMgr->GetVehicleMgr()->HasVehicleWeapon() )
	{
		m_pMoveMgr->GetVehicleMgr()->UpdateVehicleWeapon( rRot, vPos, bFire, eFireType );
	}
	else
	{
...
{
	float fRecoilKick = GetConsoleFloat("CamRecoilKick",-1.0f);
	float fRecoilFactor = GetConsoleFloat("CamRecoilFactor",1.0f);
	float fRecoilScale = GetConsoleFloat("CamRecoilScale",0.05f);
	float fRecoilMax = GetConsoleFloat("CamRecoilMax",10.0f);
	...
	if (fRecoilKick < 0.0f)
	{
		...
		{
			fRecoilKick = pWeaponData->fFireRecoilPitch * pAmmoData->fFireRecoilMult;
			}
		}


	s_fRecoilDecaySpeed = 0;

	float fKickMult = 1.0f + (m_fCurrentRecoil * fRecoilFactor);
	float fDelta = fRecoilKick * fKickMult * fRecoilScale;
	m_fCurrentRecoil += fDelta;
	float fPitchDiff = GetRandom(-fDelta,fDelta / 2.0f);
	if (GetConsoleBool("CamRecoilDebug",false))
		fPitchDiff = -fDelta / 2.0f;
	m_fPitch += DEG2RAD(fPitchDiff);
	...
	float fYawDiff = GetRandom(-fDelta,fDelta) / 3.0f;
	if (GetConsoleBool("CamRecoilDebug",false))
		fYawDiff = 0.0f;
	m_fYaw += DEG2RAD(fYawDiff);
...
void CPlayerMgr::DecayWeaponRecoil()
{
	...
	float fElapsedTime = (g_pLTClient->GetTime() - m_fLastFireTime);

	//don't decay immediately
	if (fElapsedTime < 0.03f)
		return;

	float fRecoilRecover = GetConsoleFloat("CamRecoilRecover",-1.0f);
	// Move view up a bit...(based on the current weapon/ammo type)
	if (fRecoilRecover < 0.0f)
		...
		if (pWeaponData)
				{
			fRecoilRecover = pWeaponData->fFireRecoilDecay;
				}
			}

	s_fRecoilDecaySpeed += g_pLTClient->GetFrameTime();

	float fRecover = fRecoilRecover * (s_fRecoilDecaySpeed * g_pLTClient->GetFrameTime()) * g_pPlayerStats->GetSkillModifier(SKL_AIM,AimModifiers::eCorrection);
...
LTBOOL CPlayerMgr::OnCommandOn(int command)
{
	...
	switch (command)
	{
		case COMMAND_ID_ACTIVATE :
		{
			...
			else
			{
				...
				else if (GetTargetMgr()->IsTargetInRange())
				{
					...
					else if( g_pDamageFXMgr->AllowWeapons( ) && GetTargetMgr()->GetTargetObject() && DoActivate())
					{
						return TRUE;
					}
					else if( GetCarryingObject() && CanDropCarriedObject( ))
					{
				...
				// If we're carrying an object that disallows weapons, we can automatically
				// drop it with activate.
				else if( GetCarryingObject() && CanDropCarriedObject( ) && g_pPlayerMgr->IsCarryingHeavyObject())
				{
					CActivationData data = GetTargetMgr()->GetActivationData();
					data.m_nType = MID_ACTIVATE_MOVE;
					CAutoMessage cMsg;
		...
		case COMMAND_ID_MOVE_BODY:
		{
			...
			// If we're carrying something, then drop it.
			...
			// If we can pick something up, then do it.
			else if (GetTargetMgr()->IsTargetInRange())
			{				
				if( CanCarryObject( NULL ))
				{
					CActivationData data = GetTargetMgr()->GetActivationData();
					data.m_nType = MID_ACTIVATE_MOVE;
					CAutoMessage cMsg;
...
//			g_pScores->Show(false);
			...
			{
				// We're comming alive from a respawn so make sure we set the appropriate rotation...
...
{
	// If the player is alive, or in the middle of the respawn process, don't do anything special.
	// Just treat like single player....
...
void CPlayerMgr::CalculateCameraRotation()
{
	...
	if(!(m_dwPlayerFlags & BC_CFLG_STRAFE))
	{
		...
		{
			fYawDelta = -1.0f * g_pGameClientShell->GetFrameTime() * ((m_dwPlayerFlags & BC_CFLG_RUN) ? g_vtFastTurnRate.GetFloat() : g_vtNormalTurnRate.GetFloat());
		}
		if(m_dwPlayerFlags & BC_CFLG_RIGHT)
		{
			fYawDelta = g_pGameClientShell->GetFrameTime() * ((m_dwPlayerFlags & BC_CFLG_RUN) ? g_vtFastTurnRate.GetFloat() : g_vtNormalTurnRate.GetFloat());
		}
	}

		m_fYaw += fYawDelta;

	float fTempPitch = m_fPitch;
		...
		|| m_bHoldingMouseLook)
    {
		...
		{
			fTempPitch -= g_pGameClientShell->GetFrameTime() * g_vtLookUpRate.GetFloat();
		}
		...
		if (!m_pPlayerCamera->IsFirstPerson())
		{
            static float const fMinY = DEG2RAD(45.0f) - 0.1f;

			if (fTempPitch < -fMinY) fTempPitch = -fMinY;
			if (fTempPitch > fMinY)  fTempPitch = fMinY;
		}
	}
	else if (fTempPitch != 0.0f && GetConsoleInt("AutoCenter",0))
	{
        float fPitchDelta = (g_pGameClientShell->GetFrameTime() * g_vtLookUpRate.GetFloat());
		if (fTempPitch > 0.0f) fTempPitch -= Min(fPitchDelta, fTempPitch);
		if (fTempPitch < 0.0f) fTempPitch += Min(fPitchDelta, -fTempPitch);
	}

    static float const fMinY = MATH_HALFPI - 0.1f;
	fTempPitch = Clamp( fTempPitch, -fMinY, fMinY );

	// Calculate the final pitch delta and set the final pitch.
	fPitchDelta = fTempPitch - m_fPitch;
	m_fPitch = fTempPitch;
	...
	if (bIsVehicle)
	{
		...
		m_pMoveMgr->GetVehicleMgr()->CalculateVehicleRotation( vPlayerPYR, vPYR, fYawDelta, fPitchDelta );
...
void CPlayerMgr::UpdatePlayerInfo(bool bPlaying)
{
	...
	if ( m_pMoveMgr->IsInWorld() && (!g_pClientMultiplayerMgr->IsConnectedToRemoteServer( ) || fSendDelta > fSendRate))
	{
		...
		cMsg.Writeuint8(MID_PLAYER_UPDATE);
 		...
 		uint8 nCurCompRot, nCurCompPitch;

 		LTRotation rPlayerRot(m_fPlayerPitch, m_fPlayerYaw, m_fPlayerRoll);
 		nCurCompRot = CompressRotationByte( &rPlayerRot );
 		nCurCompPitch = CompressAngleToByte( m_fPitch );

		if (g_vtPlayerRotate.GetFloat(1.0) > 0.0)
		{
			PlayerPhysicsModel PPM = m_pMoveMgr->GetVehicleMgr()->GetPhysicsModel();
			if( PPM_VEHICLE == PPM )
			{
			...
			else
			{
 				...
 				if (bSendUnique || (nCurCompRot != m_nPlayerInfoLastCompRot) || (nCurCompPitch != m_nPlayerInfoLastCompPitch))
 					m_nPlayerInfoChangeFlags |= CLIENTUPDATE_PLAYERROT;
 				else
 					m_nPlayerInfoChangeFlags &= ~CLIENTUPDATE_PLAYERROT;
		...
		if (m_nPlayerInfoChangeFlags & CLIENTUPDATE_PLAYERROT)
		{
			// Set the player's rotation (don't allow model to rotate up/down).
 			cMsg.Writeuint8(nCurCompRot);
 			m_nPlayerInfoLastCompRot = nCurCompRot;
		}
		...
		if ( m_nPlayerInfoChangeFlags & ( CLIENTUPDATE_PLAYERROT | CLIENTUPDATE_ACCURATEPLAYERROT ) )
		{
			...
			// and write it to the message.
 			cMsg.Writeuint8( nCurCompPitch );
 			m_nPlayerInfoLastCompPitch = nCurCompPitch;
		}
		...
		// Write position info...
 		...
 		bool bMsgChange = bSendUnique || !AreMessagesEqual(cNewMsg, m_cLastPlayerInfoMsg);
 		// Don't send the message if it's the same as last time.
 		if (bMsgChange)
 		{
 			m_cLastPlayerInfoMsg = cNewMsg;
 			g_pLTClient->SendToServer(cNewMsg, 0);
...
//	ROUTINE:	CPlayerMgr::IsFirstPerson()
//
//	PURPOSE:	See if we are going into first person mode...
//
// --------------------------------------------------------------------------- //

LTBOOL CPlayerMgr::IsGoingFirstPerson()
{
	return m_pPlayerCamera->IsGoingFirstPerson();
}
...
void CPlayerMgr::SetCarryingObject( uint8 nCarry, ChassisPieceBute* pChassisPieceBute, bool bUpdateCameraDip )
{	
	if( m_nCarryingObject != nCarry )
	{
		m_nCarryingObject = nCarry;
		m_pCarryChassisPieceBute = pChassisPieceBute;

		bool bHeavy = false;

		if(nCarry == CFX_CARRY_BODY )
			bHeavy = true;
		else if( nCarry == CFX_CARRY_CHASSIS_PIECE )
{	
			if( pChassisPieceBute )
	{
				bHeavy = pChassisPieceBute->m_bHeavy;
			}
		}
...
uint8 CPlayerMgr::CanCarryObject( CSpecialFX** ppSFX )
{
	...
	{				
		...
		ChassisPieceFX* const pFX = (ChassisPieceFX*)g_pGameClientShell->GetSFXMgr()->FindSpecialFX(SFX_CHASSISPIECE_ID, GetTargetMgr()->GetTargetObject());
		if( pFX )
		{
			if( ppSFX )
				*ppSFX = pFX;

			return CFX_CARRY_CHASSIS_PIECE;
		}
	}
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CPlayerMgr::IsCarryingHeavyObject
//
//	PURPOSE:	Determines if we're carrying heavy object.
//
// ----------------------------------------------------------------------- //
bool CPlayerMgr::IsCarryingHeavyObject() const 
	{
	if( m_nCarryingObject == CFX_CARRY_BODY )
		return true;
	...
	if( m_nCarryingObject == CFX_CARRY_CHASSIS_PIECE )
	{
		...
		return m_pCarryChassisPieceBute->m_bHeavy;
	}
