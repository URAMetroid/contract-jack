...
void CScreenAudio::OnFocus(LTBOOL bFocus)
{
	...
	if (bFocus)
	{
		...
		m_bSoundQuality = (pProfile->m_sPerformance.nSettings[kPerform_SoundQuality] > 0);
		...
		m_bMusicQuality = (pProfile->m_sPerformance.nSettings[kPerform_MusicQuality] > 0);
...
void CScreenAudio::SaveSoundSettings()
{
	...
	bool bActive = ((m_nMusicVolume > MUSIC_MIN_VOL) && m_bMusicEnabled);
	WriteConsoleInt("MusicActive", !!bActive);
	pProfile->m_fSpeechSoundMultiplier = m_nSpeechVolume / 100.0f;
	pProfile->m_sPerformance.nSettings[kPerform_MusicQuality] = m_bMusicQuality;
	pProfile->m_sPerformance.nSettings[kPerform_SoundQuality] = m_bSoundQuality;
	pProfile->m_sPerformance.nSettings[kPerform_MusicActive] = bActive;

	pProfile->ApplySound();

	//need to ApplyPerformance() becaue music quality is a performance setting.
	pProfile->ApplyPerformance(false);
}
