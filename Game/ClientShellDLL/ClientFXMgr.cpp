...
bool CClientFXMgr::UpdateAllActiveFX(LTBOOL bAppHasFocus)
{
	...
	while( pInstNode )
	{
		...
		if( pInst->m_hParent )
		{
			g_pLTClient->GetObjectPos( pInst->m_hParent, &pInst->m_vPos );
			g_pLTClient->GetObjectRotation( pInst->m_hParent, &pInst->m_rRot );
		}
...
CLIENTFX_INSTANCE* CClientFXMgr::CreateClientFX(const CLIENTFX_CREATESTRUCT &fxInit, LTBOOL bStartInst, bool bAddNextUpdate)
{
	...
	for(uint32 nCurrKey = 0; nCurrKey < pRef->m_nNumKeys; nCurrKey++)
	{
		...
		if(!m_bGoreEnabled && ( pRef->m_pKeys[nCurrKey].m_eGoreSetting == FX_KEY::eGoreSetting_Yes ))
		{
			continue;
		}
		// If gore is enabled, then make sure the effect isn't fore lowviolence only.
		else if(m_bGoreEnabled && ( pRef->m_pKeys[nCurrKey].m_eGoreSetting == FX_KEY::eGoreSetting_LowViolenceOnly ))
		{
			continue;
		}
