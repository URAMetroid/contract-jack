...
LTBOOL CGlobalClientMgr::Init()
{
	...
	if( bOk )
	{
		...
		if( !m_pClientButeMgr || !m_pClientButeMgr->Init())
		{
			bOk = false;
	...
	if( bOk )
	{
		...
		if( !m_pSearchItemMgr || !m_pSearchItemMgr->Init())
		{
			bOk = false;
	...
	if( bOk )
	{
		...
		if( !m_pPopupMgr || !m_pPopupMgr->Init())
		{
			bOk = false;
	...
	if( bOk )
	{
		...
		if( !g_pActivateTypeMgr )
		{
			...
			if( !ActivateTypeMgr.Init() )
			{
				...
				DebugCPrint(1, "ERROR in CGlobalClientMgr::Init()\nCouldn't initialize CActivateTypeMgr.\n\n" );
			}
		}
		...
		if( !g_pTriggerTypeMgr )
		{
			...
			{
				ASSERT( !"ERROR in CGlobalClienMgr::Init() Couldn't initialize CTriggerTypeMgr." );
		...
		{
			CVehicleButeMgr &VehicleButeMgr = CVehicleButeMgr::Instance( );
			if( !VehicleButeMgr.Init( ))
			{
				ASSERT( !"ERROR in CGlobalClientMgr::Init() Couldn't initialize CVehicleButeMgr." );
		...
		{
			if( !ChassisButeMgr::Instance( ).Init( ))
			{
				ASSERT( !"ERROR in CGlobalClientMgr::Init() Couldn't initialize ChassisButeMgr." );
