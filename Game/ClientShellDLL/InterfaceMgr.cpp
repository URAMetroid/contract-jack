...
{
	eVUG, eSierra, eMonolith, eJupiter, eNVidia
};

float      g_fSplashSndDuration = 0.0f;

float      g_fFovXTan = 0.0f;
float      g_fFovYTan = 0.0f;

LTVector     g_vOverlaySpriteScale(0.02f, 0.02f, 1.0f);
LTVector     g_vOverlayModelScale(1.0f, 1.0f, 1.0f);
float      g_fOverlaySpriteDist = 1.0f;
float      g_fOverlayModelDist = 0.25f;
...
#define NUM_DEMO_SCREENS 1
char  g_szDemoScreens[NUM_DEMO_SCREENS][64] =
{
	"Interface\\Menu\\Art\\EndSplash.pcx"
};
...
static LTBOOL HandleDebugKey(int key )
{
	...
	if( iID != -1 )
	{
		...
		float fCurrentLevel = g_pLTClient->GetVarValueFloat(hVar);
		++fCurrentLevel;
...
LTBOOL CInterfaceMgr::Init()
{
	...
	// Initialize the interface timers.
	m_RedInterfaceTimer.Init( g_pLayoutMgr->GetInt("Miscellaneous","Team1TimerFont"),
		(uint8)g_pLayoutMgr->GetInt("Miscellaneous","Team1TimerSize"),
		g_pLayoutMgr->GetPoint("Miscellaneous","Team1TimerPos"),
		g_pLayoutMgr->GetVector("Miscellaneous","Team1TimerColor"), CUI_HALIGN_CENTER);
	m_BlueInterfaceTimer.Init( g_pLayoutMgr->GetInt("Miscellaneous","Team2TimerFont"),
		(uint8)g_pLayoutMgr->GetInt("Miscellaneous","Team2TimerSize"),
		g_pLayoutMgr->GetPoint("Miscellaneous","Team2TimerPos"),
		g_pLayoutMgr->GetVector("Miscellaneous","Team2TimerColor"), CUI_HALIGN_CENTER);
	m_InterfaceTimer.Init( g_pLayoutMgr->GetInt("Miscellaneous","TimerFont"),
		(uint8)g_pLayoutMgr->GetInt("Miscellaneous","TimerSize"),
		g_pLayoutMgr->GetPoint("Miscellaneous","TimerPos"),
		g_pLayoutMgr->GetVector("Miscellaneous","TimerColor"), CUI_HALIGN_CENTER);
	...
	startupInfo.m_nPatchInfoDistributionId = atoi( LoadTempString( IDS_NETWORK_DISTRIBUTIONID ));
	m_pGameSpyPatchInfo = IGameSpyPatchInfo::Create( startupInfo );
	if( m_pGameSpyPatchInfo )
	{
		m_pGameSpyPatchInfo->RequestPatchInfo( );
	}
...
*/
	...
	g_pBombTarget->SetTimer( 0, 0.0f, false );
	g_pBombTarget->SetTimer( 1, 0.0f, false );
	g_pBombTarget->SetTimer( 2, 0.0f, false );
...
{
    float fApplySun = g_vtModelApplySun.GetFloat();
...
{
	// Are we still checking for patches.
	if( !m_pGameSpyPatchInfo )
		return;
	...
	switch( m_pGameSpyPatchInfo->GetPatchInfoStatus( ))
	{
		...
		case IGameSpyPatchInfo::eBrowserStatus_Complete:
		{
			...
			if( m_pGameSpyPatchInfo->GetPatchInfoResults( patchInfoResults ) &&
				patchInfoResults.m_bNewVersionAvailable && !m_MessageBox.IsVisible( ))
			{
				...
				ShowMessageBox(IDS_NEW_VERSION,&mb);
			}

			// Don't need the patch update object anymore.
			IGameSpyPatchInfo::Delete( m_pGameSpyPatchInfo );
			m_pGameSpyPatchInfo = NULL;
			break;
		}
		// Something's wrong.
...
void CInterfaceMgr::GetPatchCallBack( LTBOOL bReturn, void *pData )
{
	CInterfaceMgr* pInterfaceMgr = reinterpret_cast< CInterfaceMgr* >( pData );

	if( bReturn )
	{
		// Launch the updater.
		g_pGameClientShell->LaunchPatchUpdate( pInterfaceMgr->m_sPatchUrl.c_str( ));
	}
}
...
void CInterfaceMgr::UpdateSplashScreenState()
{
	...
	if(m_hSplashSound)
	{
		...
		{
			DebugCPrint(1,"Current Time: %.4f", g_pLTClient->GetTime());
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CInterfaceMgr::StartMenuMusic
//
//	PURPOSE:	Starts the menu music.
//
// ----------------------------------------------------------------------- //

bool CInterfaceMgr::StartMenuMusic()
{
	CMusic* pMusic = g_pGameClientShell->GetMusic();
    if( !pMusic) 
		return false;
	...
	// Don't restart the music if it isn't enabled...
	if (!m_Settings.MusicEnabled() || !GetConsoleInt("MusicActive",0))
	{
		return true;
	}
	...
	CMusicState MusicState;
	MusicState.nIntensity = g_pLayoutMgr->GetScreenMusicIntensity(GetScreenMgr( )->GetCurrentScreenID());;
	strcpy(MusicState.szDirectory, szDir);
	strcpy(MusicState.szControlFile, szFile);
	...
	pMusic->ChangeIntensity(MusicState.nIntensity);
...
static uint8 GetChassisPieceButeIndex( char const* pszName )
{
	ChassisPieceBute* pChassisPieceBute = ChassisButeMgr::Instance( ).GetChassisPieceBute( pszName );
...
static uint8 ConvertChassisPieceButeIndexToDDIndex( uint8 nChassisPieceButeIndex )
{
	static const uint8 kTransmitter = GetChassisPieceButeIndex( "Doomsday_transmitter" );
	static const uint8 kBattery = GetChassisPieceButeIndex( "Doomsday_batteries" );
	static const uint8 kCore = GetChassisPieceButeIndex( "Doomsday_core" );

	if( nChassisPieceButeIndex == kTransmitter )
		return 0;
	else if( nChassisPieceButeIndex == kBattery )
		return 1;
	else if( nChassisPieceButeIndex == kCore )
		return 2;
...
//				m_MenuMgr.GetCurrentMenu()->OnCommand(MC_UPDATE,0,0);
		...
		case MID_GEAR_PICKEDUP :
		{
			...
			bool bActivatingPickup = pMsg->Readbool( );

			g_pGameClientShell->HandleGearPickup(nGearId, bPickedUp, bActivatingPickup);
		...
		case MID_PLAYER_CONNECTED:
		{
			...
			DebugCPrint(0,"MID_PLAYER_CONNECTED : Client  %d (%s)", nID, szName);
		...
		case MID_DOOMSDAY_MESSAGE:
		{
			...
			switch( nId )
			{
				...
				case MID_DOOMSDAY_PIECE_STOLEN:
				{
					...
					struct DoomsdayActionSound
					{
						char const* m_pszActionSound[3];
					};
					static DoomsdayActionSound doomsdayActionSound[] =
					{
						// Pickedup
						{
							// Transmitter
					...
					uint8 nPiece = ConvertChassisPieceButeIndexToDDIndex( pMsg->Readuint8());
					uint8		nTeam = pMsg->Readuint8();

					if( nPiece >= kDoomsDay_MAXTYPES )
						return LTFALSE;
					if( nTeam >= MAX_TEAMS )
						return LTFALSE;
					...
					LoadString( dwPieceID[nPiece], szPiece, ARRAY_LEN(szPiece) );
					...
					// Play a sound...
					g_pClientSoundMgr->PlayInterfaceSound( doomsdayActionSound[nId].m_pszActionSound[nPiece] );
				}
				break;
				...
				case MID_DOOMSDAY_DEVICE_COMPLETED:
				{
					...
					// Play a sound...
					g_pClientSoundMgr->PlayInterfaceSound("voice\\10343.wav");
				}
				break;
...
void CInterfaceMgr::UpdatePlayerStats(ILTMessage_Read& msg )
{
	...
	switch (nThing)
	{
		...
		{
			bool bActivatingPickup = msg.Readbool( );

			if (nType2 != WMGR_INVALID_ID)
			{
				GetPlayerStats( )->UpdateAmmo(nType1, nType2, (uint32)fAmount, LTTRUE, LTTRUE, bActivatingPickup );
			}
			else
			{
				g_pGameClientShell->HandleWeaponPickup( nType1, false, bActivatingPickup );
			}
		}
		break;
		...
		case IC_MOD_PICKUP_ID :
		{
			...
			if (bDisplayMsg)
			{
				g_pGameClientShell->HandleModPickup(nType2, bPickedUp, bActivatingPickup);
			}
		...
		{
			GetPlayerStats( )->UpdateObjectives(nThing, nType1, (uint32)fAmount);
			g_pTransmission->Show(IDS_FOUND_SECRET);
			g_pChatMsgs->AddMessage(IDS_FOUND_SECRET,kMsgTransmission);
			g_pClientSoundMgr->PlayInterfaceSound("Interface\\Snd\\secret.wav");
		}
		break;
	...
	switch (command)
	{
		...
		case COMMAND_ID_NEXT_AMMO :
		{
			...
			if (m_AmmoChooser.Open())
			{
				...
				if ( pClientWeapon )
				{
					pClientWeapon->ChangeAmmoWithReload( nCurrAmmo, true );
				}
			}
		...
		{
			// Make sure the choosers are closed if we fire.
			CloseChoosers( );
		}
		break;
...
LTBOOL CInterfaceMgr::OnKeyDown(int key, int rep)
{
	...
	switch (m_eGameState)
	{
		...
		case GS_MOVIE :
		{
			...
			if (pRegMgr->IsValid())
			{
				uint32 nGameRuns = pRegMgr->Get( "GameRuns",0 );
				if( nGameRuns > 1 )
					bEndMovies = true;
			}
...
LTBOOL CInterfaceMgr::PreSplashScreenState(GameState eCurState)
{
	...
	m_hSplashSound = g_pClientSoundMgr->PlayInterfaceSound(IM_SPLASH_SOUND, dwFlags);
...
void CInterfaceMgr::ScreenDimsChanged()
{
	...
	m_InterfaceTimer.ScreenDimsChanged();
	m_RedInterfaceTimer.ScreenDimsChanged();
	m_BlueInterfaceTimer.ScreenDimsChanged();
...
void CInterfaceMgr::UpdateScreenFade(bool bUpdateAlpha /*=true*/)
{
	...
	if (m_hFadeSurface)
	{
		float fTimeDelta = m_bFadeIn ? m_fCurFadeTime : (m_fTotalFadeTime - m_fCurFadeTime);

		float fLinearAlpha = (fTimeDelta / m_fTotalFadeTime);
		fLinearAlpha = fLinearAlpha < 0.0f ? 0.0f : (fLinearAlpha > 1.0f ? 1.0f : fLinearAlpha);

		float fAlpha = 1.0f - WaveFn_SlowOn(1.0f - fLinearAlpha);
...
void CInterfaceMgr::UpdateLetterBox()
{
	...
	float fTimeLeft = m_fLetterBoxFadeEndTime - fTime;
...
void CInterfaceMgr::UpdateModelAnimations(float fCurFrameDelta)
{
...
char* CInterfaceMgr::GetCurrentMovie()
{
	...
	switch (m_nCurMovie)
	{
		...
		case eJupiter :
			pMovie = "Movies\\Jupiter.bik";
		break;

		case eVUG :
			pMovie = "Movies\\VUGlogo.bik";
		break;
...
void CInterfaceMgr::Disconnected(uint32 nDisconnectFlag)
{
	...
	switch (nDisconnectCode)
	{
		...
		case eDisconnect_WrongPassword:
			...
			destination = SCREEN_ID_MULTI;
			break;
		case eDisconnect_BadCdKey:
			nMsgId = IDS_NETERR_VERIFYCDKEYFAIL;
			destination = SCREEN_ID_MULTI;
			break;
		case eDisconnect_InvalidAssets:
			nMsgId = IDS_NETERR_INVALIDASSETS;
			destination = SCREEN_ID_MULTI;
			break;
		case eDisconnect_Banned:
			nMsgId = IDS_NETERR_BANNED;
			destination = SCREEN_ID_MULTI;
			break;
...
void CInterfaceMgr::ConnectionFailed(uint32 nConnectionError)
{
	...
	if (IsMultiplayerGame())
	{
		bool bWasHosting = g_pClientMultiplayerMgr->GetStartGameRequest( ).m_Type == STARTGAME_HOST;
		switch( g_pClientMultiplayerMgr->GetServerGameOptions( ).m_eServerStartResult )
		{
			default:
				break;
			...
			case eServerStartResult_NetworkError:
				nMsgId = IDS_NETERR_SELECTSERVICE;
				break;
		}
	...
	{
		nMsgId = IDS_SERVERFULL;
	}

	LoadFailed( eScreenToShow, nMsgId );
}
...
LTVector CInterfaceMgr::GetWorldFromScreenPos(LTIntPt pos, float fDepth)
{
	LTVector vPos;

	float fx = (2.0f * ((float)pos.x / (float)g_pInterfaceResMgr->GetScreenWidth() )) - 1.0f;
	float fy = 1.0f - (2.0f * ((float)pos.y / (float)g_pInterfaceResMgr->GetScreenHeight()) );
...
LTVector CInterfaceMgr::GetScreenFromWorldPos(LTVector vPos, HOBJECT hCamera)
{
	...
	float fx = tmp.x / (g_fFovXTan * tmp.z);
	float fy = tmp.y / (g_fFovYTan * tmp.z);

	pos.x =  ((float)g_pInterfaceResMgr->GetScreenWidth() * (1.0f + fx) / 2.0f);
	pos.y =	 ((float)g_pInterfaceResMgr->GetScreenHeight() * (1.0f - fy) / 2.0f);
	pos.z =  tmp.z;
...
bool CInterfaceMgr::HandleDisplayTimerMsg( ILTMessage_Read& msg )
{
	...
	switch( nId )
	{
		...
		case 4:
		case 5:
			g_pBombTarget->SetTimer( nId - 3, fGameTimeEnd, bPaused );
			break;
	}
    return LTTRUE;
}
