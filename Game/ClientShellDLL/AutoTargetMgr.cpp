...
static bool DoVectorPolyFilterFn(HPOLY hPoly, void *pUserData)
{
	...
	if (pSurf && pSurf->bCanShootThrough && pSurf->bCanSeeThrough) 
		return false;
...
void CAutoTargetMgr::Update()
{
	...
	const WEAPON* pWeapon = pClientWeapon->GetWeapon();

	if( g_pMoveMgr->GetVehicleMgr()->IsVehiclePhysics() && g_pMoveMgr->GetVehicleMgr()->HasVehicleWeapon())
	{
		pWeapon = g_pMoveMgr->GetVehicleMgr()->GetVehicleWeapon()->GetWeapon();
	}

	if (pWeapon->eUseAutoTarget == AT_NEVER || (pWeapon->eUseAutoTarget == AT_EASY && g_pGameClientShell->GetDifficulty() != GD_EASY) )
		return;
	...
	m_fAngle = pWeapon->fAutoTargetAngle;
	m_fRangeSqr = pWeapon->fAutoTargetRange*pWeapon->fAutoTargetRange;
	...
	if (hCamera)
	{
		g_pLTClient->GetObjectPos(hCamera, &m_vFirePos);
...
void CAutoTargetMgr::GenerateNodeArray()
{
	...
	while (iter != m_Targets.end())
	{
		...
		cNodes = g_pModelButeMgr->GetSkeletonNumNodes(eModelSkeleton);
	
		// Enumerate through the nodez
		for(int iNode = 0; iNode < cNodes && m_nNodeCount < MAX_AUTOTARGET_NODES; iNode++)
		{
			eCurrentNode = (ModelNode)iNode;
			if (g_pModelButeMgr->GetSkeletonNodeAutoTarget(eModelSkeleton, eCurrentNode))
			{
				...
				{
					if( LT_OK == pModelLT->GetNodeTransform( pChar->GetServerObj(), hNode, lTrans, LTTRUE ) )
					{	
						m_NodeArray[m_nNodeCount].vPos = lTrans.m_Pos;
...
bool CAutoTargetMgr::IsPointInCone(const LTVector &vTargetPos)
{
	...
	vecD.Normalize();
	static float MaxScreenAngle = (float)cos( DEG2RAD(g_vtFOVYNormal.GetFloat()) / 2.0);
...
LTVector CAutoTargetMgr::GetCrosshairPos() const
{
	...
	LTVector pos = g_pInterfaceMgr->GetScreenFromWorldPos(vTemp, g_pPlayerMgr->GetCamera());
	return pos;
}
...
void CAutoTargetMgr::InterpolateAim()
{
	...
	static float fMPAutoTargetSpeed = g_pClientButeMgr->GetInterfaceAttributeFloat("MPAutoTargetSpeed", 0.5f);
	static float fAutoTargetSpeed = g_pClientButeMgr->GetInterfaceAttributeFloat("AutoTargetSpeed", 1.0f);

	//get the maximum angle we can move
	float fMaxAngVel = g_pLTClient->GetFrameTime();
	if (IsMultiplayerGame())
	{
		fMaxAngVel *= fMPAutoTargetSpeed;
	}
	else
	{
		fMaxAngVel *= fAutoTargetSpeed;
	}
