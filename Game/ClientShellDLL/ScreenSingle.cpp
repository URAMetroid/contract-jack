...
{
	"worlds\\retailsingleplayer\\m02s01",
	"worlds\\retailsingleplayer\\m04s01",
};
...
#if !defined(_SPDEMO) && !defined(_PRDEMO)
	AddTextItem(IDS_NEWGAME, CMD_NEW_GAME, IDS_HELP_NEW);
#else // !defined(_SPDEMO) && !defined(_PRDEMO)

	int nCmdOffset = 0;
	int nLevelIndex;
	int nMission;
	for( uint32 nLevel = 0; nLevel < s_nNumLevels; nLevel++ )
	{
		if( !g_pMissionButeMgr->IsMissionLevel( s_apszLevels[nLevel], nMission, nLevelIndex ))
			return LTFALSE;

		MISSION* pMISSION = g_pMissionButeMgr->GetMission( nMission );
		AddTextItem( LoadTempString( pMISSION->nNameId ), CMD_LEVEL1 + nCmdOffset, 0);
		nCmdOffset++;
	}

#endif // !defined(_SPDEMO) && !defined(_PRDEMO)
	...
	case CMD_LEVEL3:
		{
			...
			int nMission;
			int nLevelIndex;
			if( !g_pMissionButeMgr->IsMissionLevel( s_apszLevels[nSelectedLevel], nMission, nLevelIndex ))
				return 1;

			LEVEL* pLevel = g_pMissionButeMgr->GetLevel( nMission, nLevelIndex );
			if( pLevel && g_pClientMultiplayerMgr->SetupServerSinglePlayer( ))
			{
				g_pMissionMgr->StartGameFromLevel( pLevel->szLevel );
			}
