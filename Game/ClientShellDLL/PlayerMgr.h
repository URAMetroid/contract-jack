...
public:
	...
	ChassisPieceBute* GetCarryingChassisPiece( ) const { return m_pCarryChassisPieceBute; }
	bool		IsCarryingHeavyObject() const;
	void		SetCarryingObject( uint8 nCarry, ChassisPieceBute* pChassisPieceBute, bool bUpdateCameraDip );
	uint8		CanCarryObject( CSpecialFX** pSFX );
...
protected:
    ...
    float		m_fPlayerInfoLastSendTime, m_fPlayerInfoLastUniqueSendTime;
	uint8		m_nPlayerInfoLastCompRot, m_nPlayerInfoLastCompPitch;
	CLTMsgRef_Read m_cLastPlayerInfoMsg, m_cLastPlayerInfoMoveMsg;
	...
	ChassisPieceBute* m_pCarryChassisPieceBute;
	...
	// Saves the rotation comming from the server to use when needed...
	LTVector	m_vServerRot;
};
