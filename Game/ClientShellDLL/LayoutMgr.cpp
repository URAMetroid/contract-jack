...
#define	LO_HUD_WEAPONHEAT_TEX			"WeaponHeatTex"
#define	LO_HUD_WEAPONHEAT_BASEPOS		"WeaponHeatBasePos"
#define	LO_HUD_WEAPONHEAT_BAR_HEIGHT	"WeaponHeatBarHeight"
#define	LO_HUD_WEAPONHEAT_BAR_SCALE		"WeaponHeatBarScale"
#define	LO_HUD_WEAPONHEAT_COOL			"WeaponHeatCool"
#define	LO_HUD_WEAPONHEAT_WARM			"WeaponHeatWarm"
#define	LO_HUD_WEAPONHEAT_HOT			"WeaponHeatHot"
#define	LO_HUD_WEAPONHEAT_OVERHEAT		"WeaponHeatOverheat"
...
void CLayoutMgr::GetWeaponHeatBarTexture( int nLayout, char *pBuf, int nBufLen )
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if (!m_buteMgr.Exist(s_aTagName,LO_HUD_WEAPONHEAT_TEX))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	GetString( s_aTagName, LO_HUD_WEAPONHEAT_TEX, pBuf, nBufLen );
}


LTIntPt CLayoutMgr::GetWeaponHeatBasePos(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
...
int CLayoutMgr::GetWeaponHeatBarHeight(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if (!m_buteMgr.Exist(s_aTagName,LO_HUD_WEAPONHEAT_BAR_HEIGHT))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	return GetInt(s_aTagName, LO_HUD_WEAPONHEAT_BAR_HEIGHT);
}

LTFLOAT CLayoutMgr::GetWeaponHeatBarScale(int nLayout)
{
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if (!m_buteMgr.Exist(s_aTagName,LO_HUD_WEAPONHEAT_BAR_SCALE))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	return GetFloat(s_aTagName, LO_HUD_WEAPONHEAT_BAR_SCALE);
}


void CLayoutMgr::GetWeaponHeatColors( int nLayout, LTVector& vCool, LTVector& vWarm, LTVector& vHot, LTVector& vOverheat )
{
	LTVector vColor;
	sprintf(s_aTagName, "%s%d", LO_HUD_TAG, nLayout);
	if( !m_buteMgr.Exist(s_aTagName, LO_HUD_WEAPONHEAT_COOL))
		sprintf(s_aTagName, "%s0", LO_HUD_TAG);

	vCool = m_buteMgr.GetVector(s_aTagName, LO_HUD_WEAPONHEAT_COOL);
	vWarm = m_buteMgr.GetVector(s_aTagName, LO_HUD_WEAPONHEAT_WARM);
	vHot = m_buteMgr.GetVector(s_aTagName, LO_HUD_WEAPONHEAT_HOT);
	vOverheat = m_buteMgr.GetVector(s_aTagName, LO_HUD_WEAPONHEAT_OVERHEAT);
}
