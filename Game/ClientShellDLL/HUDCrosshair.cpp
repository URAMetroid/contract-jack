...
void CHUDCrosshair::Render()
{
	...
	if( !bRenderScope )
	{
		...
		else if (CAutoTargetMgr::Instance().IsLockedOn())
		{
			...
			LTVector pos = CAutoTargetMgr::Instance().GetCrosshairPos();
		...
		{
			float cx = (float)g_pInterfaceResMgr->GetScreenWidth() / 2.0f;
			float cy = (float)g_pInterfaceResMgr->GetScreenHeight() / 2.0f;
			float w = m_fAccurateSz;
			float x = (float)cx - w/2.0f;
			float y = (float)cy - w/2.0f;
			g_pDrawPrim->SetXYWH(&m_Poly[0],x,y,w,w);
