...
void CTO2PlayerStats::UpdateAmmo(uint8 nWeaponId, uint8 nAmmoId,
                              ...
                              LTBOOL bDisplayMsg, bool bActivatingPickup)
{
	...
	if (bPickedup && g_pWeaponMgr->IsValidWeaponId(nWeaponId))
	{
		if (m_pbHaveWeapon)
		{
			...
			// if (!m_pbHaveWeapon[nWeaponId])
			{
				if (bDisplayMsg)
				{
					g_pGameClientShell->HandleWeaponPickup(nWeaponId, true, bActivatingPickup);
				}
			}
		...
		if (m_pnAmmo)
		{
			if (bPickedup) /*** BL 12/08/2000 Added to equip weapon when you are out of ammo and pick up ammo/weapon ******/
			{
				...
				for ( int32 iWeapon = g_pWeaponMgr->GetNumWeapons()-1 ; iWeapon >= 0 ; iWeapon-- )
				{
					...
					if ( nWeapon != WMGR_INVALID_ID && m_pbHaveWeapon[nWeapon] )
					{
						...
						if ( !pWeapon->bInfiniteAmmo && pWeapon->bShowChooser )
						{
			...
			if (bPickedup && bDisplayMsg)
			{
				...
				if (!pWeapon || !(pWeapon->bInfiniteAmmo))
				{
					...
					g_pGameClientShell->HandleAmmoPickup(nAmmoId, taken, bTookAmmo, bActivatingPickup, nWeaponId);
				}
			}
	...
	bool bForce = (!g_pPlayerMgr->GetCurrentClientWeapon() && 
					g_pPlayerMgr->GetClientWeaponMgr()->GetDefaultWeaponId() == nEquipWeapon);
	if ( nEquipWeapon != WMGR_INVALID_ID && (bForce || GetConsoleBool("AutoWeaponSwitch",true)) )  /*** BL 12/08/2000 Added to change equip weapon when you are out of ammo and pick up a weapon ******/
	{
...
void CTO2PlayerStats::Load(ILTMessage_Read *pMsg)
{
	...
	// Add the keys to the hud.
	IDList::IDArray::iterator iter = m_Keys.m_IDArray.begin( );
	for( ; iter != m_Keys.m_IDArray.end( ); iter++ )
	{
		g_pCarrying->AddKey( *iter );
	}
