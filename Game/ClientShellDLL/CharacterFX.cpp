	...
	else if( stricmp( pKey, RENDERSTYLE_MODEL_KEY ) == 0 )
	{
				...
				('\0' != pArgs->argv[i][0]))
		{
			...
			if(stricmp(pArgs->argv[i],RENDERSTYLE_MODEL_KEY) == 0)
			{
				...
				{
					// Not enough params
					DebugCPrint(1,"CCharacterFX::OnModelKey - ERROR - Not enough RS arguments! Syntax: RS <RSNum> <RSName>\n");
				}
...
void CCharacterFX::DoFootStepKey(HLOCALOBJ hObj, LTBOOL bForceSound)
{
	...
	if (bPlaySound)
	{
		PlayMovementSound( vPos, m_eLastSurface, m_bLeftFoot, m_cs.ePlayerPhysicsModel, m_cs.nVehicleId );
	}
...
void CCharacterFX::PlayMovementSound(LTVector vPos, SurfaceType eSurface, LTBOOL bLeftFoot,
									 PlayerPhysicsModel ePPModel /*= PPM_NORMAL*/, uint8 nModelId /*= (uint8)-1*/ )
{
    CMoveMgr* pMoveMgr = g_pPlayerMgr->GetMoveMgr();
	if (!pMoveMgr) return;
	...
	const char *pSound = GetMovementSound( eSurface, bLeftFoot, ePPModel, nModelId );
	...
	if (pSound && *pSound)
	{
		...
		LTFLOAT fStealth = (ePPModel == PPM_NORMAL ? m_cs.fStealthPercent : 1.0f);
...
void CCharacterFX::UpdateMultiVehicleSounds()
{
	if (m_bOnVehicle)
	{
		...
		if (!m_hVehicleSound)
		{
			...
			if( pVehicle )
			{
				g_pClientSoundMgr->PlaySoundFromObject(m_hServerObject, pVehicle->sStartUpSnd.c_str(),
				...
				m_hVehicleSound = g_pClientSoundMgr->PlaySoundFromObject(m_hServerObject,
					pVehicle->sMPLoop.c_str(), DEFAULT_VEHICLE_RADIUS, SOUNDPRIORITY_PLAYER_HIGH, dwFlags);
			}
		}
		else
		{
...
LTBOOL CCharacterFX::OnServerMessage(ILTMessage_Read *pMsg)
{
	...
	switch(nMsgId)
	{
		...
		case CFX_CARRY:
		{
			...
			uint8 nChassisPieceIndex = pMsg->Readuint8( );
			if( nChassisPieceIndex != CHASSISPIECEBUTE_INVALID )
				m_cs.pChassisPieceBute = ChassisButeMgr::Instance( ).GetChassisPieceButeList( )[nChassisPieceIndex];
			else
				m_cs.pChassisPieceBute = NULL;
		...
		case CFX_PLAYER_PHYSICS_MODEL:
		{
			 ...
			 m_cs.nVehicleId		= pMsg->Readuint8();

			// If getting off of a vehicle handle some cleanup and shutdown...

			if( (m_cs.ePlayerPhysicsModel == PPM_VEHICLE) && (ppm != PPM_VEHICLE) )
			{
				if(  m_hVehicleSound )
				{
					g_pLTClient->SoundMgr()->KillSound(m_hVehicleSound);
				...
				if( pVehicle )
				{
					g_pClientSoundMgr->PlaySoundFromObject(m_hServerObject, pVehicle->sTurnOffSnd.c_str(),
						DEFAULT_VEHICLE_RADIUS, SOUNDPRIORITY_PLAYER_HIGH);
				}
			}
			...
			m_bOnVehicle = (m_cs.ePlayerPhysicsModel == PPM_VEHICLE);
		}
		break;
...
// ----------------------------------------------------------------------- //
//
//  ROUTINE:	CCharacterFX::HandlePitchData
//
//  PURPOSE:	Update the tracking nodes for pitching
//
// ----------------------------------------------------------------------- //

void CCharacterFX::HandlePitchData( uint32 nPitch )
{
	...
	// remap it to a float
	UncompressAngleFromByte( (uint8)nPitch, &m_cs.fPitch );
...
void CCharacterFX::UpdatePitch( )
{
	...
	uint32 nPitch;
	if( g_pLTBase->GetObjectUnguaranteedData( m_hServerObject, &nPitch) == LT_OK )
	{
		HandlePitchData( nPitch );
	}
...
void CCharacterFX::UpdateCarrying()
{
	...
	g_pPlayerMgr->SetCarryingObject(m_cs.nCarrying, m_cs.pChassisPieceBute, true);
}
