		...
		for (int i = 0; i < m_nNumChoices; i++)
		{
			if (m_pText[i])
			{
				...
				case eGameTypeDemolition:
					m_pText[i]->GetColumn(1)->SetString(LoadTempString(g_DERadioArray[i]));
					break;
				default:
...
void CHUDRadio::Choose(uint8 nChoice)
{
	...
	case eGameTypeDoomsDay:
		...
		break;
	case eGameTypeDemolition:
		g_pClientMultiplayerMgr->DoTaunt(nLocalID,g_DERadioArray[nChoice]);
		break;
	default:
...
//		m_pText[i]->SetFixedWidth(nTextWidth);
	...
	{	m_Dlg.SetScale(2.749 /*URA2 g_pInterfaceResMgr->GetXRatio()*/);}
