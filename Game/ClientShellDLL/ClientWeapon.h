...
public:
	...
	virtual bool          IsGadget() const { return ( m_pAmmo && IsGadgetType(m_pAmmo->eInstDamageType)  ); }
...
protected:
	...
	virtual bool          CreateWeaponModel();
	virtual void          RemoveWeaponModel();
	...
	virtual bool	PlayReloadAnimation();
	...
	// overheat flash helper routines
	void          CreateOverheatFx();
	void          RemoveOverheatFx();

	// Updates all muzzle effects.
	void          UpdateMuzzleEffects( WeaponState eState, LTVector const &vOffset );
	...
	LTFLOAT       m_fFireTime;       // Time of last fire.
	...
	// Used to track overheating weapons.
	float			m_fHeatAmount;
	float			m_fHeatTime;
	CLIENTFX_LINK	m_OverheatFx;  // An instance of a FxED created FX 
};
