...
public:
	...
	void		GetWeaponHeatBarTexture( int nLayout, char *pBuf, int nBufLen );
    LTIntPt     GetWeaponHeatBasePos(int nLayout);
	int			GetWeaponHeatBarHeight(int nLayout);
    LTFLOAT     GetWeaponHeatBarScale(int nLayout);
	void		GetWeaponHeatColors( int nLayout, LTVector& vCool, LTVector& vWarm, LTVector& vHot, LTVector& vOverheat );
