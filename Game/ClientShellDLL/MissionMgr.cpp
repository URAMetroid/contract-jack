...
{
	// If we're told to exit the level while we're dead or dying, make sure we go to the 
	// mission failed screen, not to the next level.
	if( !IsMultiplayerGame( ))
	{
		if( g_pPlayerMgr->GetPlayerState( ) == PS_DEAD || g_pPlayerMgr->GetPlayerState( ) == PS_DYING )
		{
			g_pPlayerMgr->ClearPlayerModes();
			// [KLS 7/28/02] - Interface mgr handles forcing screen fading in
			// if necessary, doing it here may cause problems...
