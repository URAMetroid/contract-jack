...
{
	if (dwCommand >= CMD_CUSTOM && dwCommand <= CMD_CUSTOM+2+kMaxSave)
	{
		bool bLoaded = true;
									...
									bool bUserName, LTIntPt pos )
{
	...
	CLTGUIColumnCtrl* pColCtrl = CreateColumnCtrl(nCommandId, nControlHelpStringId, pos );
...
void CScreenLoad::BuildSavedLevelLists()
{
	...
	if (g_pClientSaveLoadMgr->QuickSaveExists())
	{
		...
		pos = m_nextPos;
		m_nextPos.x += kIndent;
		CLTGUIColumnCtrl*  pColCtrl = BuildSaveControls( QUICKSAVE_INIKEY, CMD_CUSTOM, IDS_HELP_QUICKLOAD, false, m_nextPos );
		AddControl(pColCtrl);
		m_nextPos.x -= kIndent;
	}
	...
	if (g_pClientSaveLoadMgr->ReloadSaveExists())
	{
		...
		pos = m_nextPos;
		m_nextPos.x += kIndent;
		CLTGUIColumnCtrl*  pColCtrl = BuildSaveControls( RELOADLEVEL_INIKEY, CMD_CUSTOM+1, IDS_HELP_RELOAD, false, m_nextPos );
		AddControl(pColCtrl);
		m_nextPos.x -= kIndent;
	}
	...
	if (g_pClientSaveLoadMgr->CheckpointSaveExists())
	{
		...
		CLTGUITextCtrl *pCtrl = AddTextItem( IDS_LOADCHECKPOINT,LTNULL,LTNULL,kDefaultPos,LTTRUE);
		pCtrl->Enable(LTFALSE);
		pos = m_nextPos;
		m_nextPos.x += kIndent;
		CLTGUIColumnCtrl*  pColCtrl = BuildSaveControls( CHECKPOINTSAVE_INIKEY, CMD_CUSTOM+2, IDS_HELP_LOADCHECKPOINTSAVE, false, m_nextPos );
		AddControl(pColCtrl);
		m_nextPos.x -= kIndent;
	}
	...
	if (strlen(pSG->szUserName) == 0)
	{
		if (mission >= 0)
		{
			...
			if (pMission)
			{
				...
				//strip whitespace
				char *pszLevel = szLevel;
				while (*pszLevel == ' ' || *pszLevel == '\t')
					pszLevel++;

				if(strlen(pszLevel))
					sprintf(pSG->szUserName,"%s",pszLevel);
				else
