                                     ...
                                     bool bPlayDeselect /*=true*/ )
{
	...
	if( !CanChangeToWeapon( nWeaponId ) )
	{
		return false;
	}
	...
	if ( bChangeWeapon )
	{
		...
		if ( bSwitchInstantly )
		{
			...
			if( m_iCurrentWeapon != CWM_NO_WEAPON )
			{
...
bool CClientWeaponMgr::SetDefaultWeapon( uint8 nWeaponId )
{
	...
	if (NULL == GetCurrentClientWeapon())
		ChangeWeapon(m_nDefaultWeaponId);
	return true;
}
...
void CClientWeaponMgr::ToggleHolster( bool bPlayDeselect )
{
	...
	if( m_pCurrentWeapon )
	{
		...
		if ( g_pWeaponMgr->GetHolsterWeapon( ) == m_pCurrentWeapon->GetWeapon( ))
		{
		...
		else
		{
			...
			if( g_pWeaponMgr->GetHolsterWeapon( ))
			{
				ChangeWeapon( g_pWeaponMgr->GetHolsterWeapon( )->nId , WMGR_INVALID_ID, -1, bPlayDeselect );
			}
		}
	}
}
