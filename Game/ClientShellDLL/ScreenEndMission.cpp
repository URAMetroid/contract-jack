...
LTBOOL CScreenEndMission::Build()
{
	...
	m_pSecrets = CreateTextItem(" ",LTNULL,LTNULL,kDefaultPos,LTTRUE);
	m_pSecrets->SetFont(NULL,statSize);
	m_pList->AddControl(m_pSecrets);
...
void CScreenEndMission::OnFocus(LTBOOL bFocus)
{
	...
	if (bFocus)
	{
		...
		if (pStats)
		{
			...
			if (pMission->nNumSecrets)
			{
				m_pSecrets->SetString(FormatTempString(IDS_M_STAT_SECRETS,pStats->dwNumSecrets,pMission->nNumSecrets));
				m_pSecrets->Show(true);
			}
			else
				m_pSecrets->Show(false);
