	...
	else if( tok == s_cTok_RENDERSTYLE_MODEL_KEY )
	{
		...
		while((i < pArgList->argc) && ('\0' != pArgList->argv[i][0]))
		{
			...
			if(stricmp(pArgList->argv[i],RENDERSTYLE_MODEL_KEY) == 0)
			{
				...
				{
					// Not enough params
					DebugCPrint(1,"CClientWeapon::OnModelKey - ERROR - Not enough RS arguments! Syntax: RS <RSNum> <RSName>\n");
				}
...
WeaponState CClientWeapon::Update( bool bFire, FireType eFireType /*=FT_NORMAL_FIRE*/)
{
	...
	// Updates the weapon heat state.  Call before muzzle effects so that it has the
	// right information.
	UpdateWeaponHeat( );

	// Update the muzzle flash...
	UpdateMuzzleEffects( eState, vFlashOffset );
...
*/
	m_fFiringPerturb = g_pPlayerMgr->GetRecoilValue();
...
void CClientWeapon::SetVisible( bool bVis /*=true*/ )
{
	...
	// Always hide the flash (it will be shown when needed)...
	if( m_OverheatFx.IsValid() )
	{
		m_OverheatFx.GetInstance()->Hide();
	}
...
void CClientWeapon::CreateSilencer()
{
	...
	if( pMod->szAttachModel && pMod->szAttachModel[0] )
	{
...
void CClientWeapon::Select()
{
	if ( W_INACTIVE == GetState() )
	{
		...
		SetVisible( ((g_pPlayerMgr->IsFirstPerson() || g_pPlayerMgr->IsGoingFirstPerson()) ? true : false) );
	}
	...
	if ( m_hObject && ( INVALID_ANI != dwSelectAni ) )
	{
		...
		if (!IsSelectAni( dwAni ) )
		{
			...
			if( hCam )
			{
				...
				UpdateMuzzleEffects( W_SELECT, LTVector(0,0,0) );
			}
		}
...
WeaponState CClientWeapon::UpdateModelState( bool bFire )
{
	...
	// Check if we can't fire because the weapon is overheated.
	if( m_fHeatAmount >= 1.0f )
	{
		bFire = false;
	}
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CClientWeapon::UpdateMuzzleEffects()
//
//	PURPOSE:	Update muzzle flash and overheat fx.
//
// ----------------------------------------------------------------------- //

void CClientWeapon::UpdateMuzzleEffects( WeaponState eState, LTVector const &vOffset )
{
	...
	if( m_OverheatFx.IsValid())
	{
		if( m_fHeatAmount < 1.0f ||
			( g_pPlayerMgr->GetPlayerState() != PS_ALIVE ) ||
			( IsLiquid( g_pPlayerMgr->GetCurContainerCode())) )
		{
			m_OverheatFx.GetInstance()->Hide();
		}
		else
		{
			// Align the flash object to the direction the model is facing...
			m_OverheatFx.GetInstance()->Show();
			m_OverheatFx.GetInstance()->SetPos( m_vFlashPos, m_vFlashOffset );
		}
	}
}
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CClientWeapon::CreateOverheatFx
//
//	PURPOSE:	Create the overheat effect
//
// ----------------------------------------------------------------------- //

void CClientWeapon::CreateOverheatFx()
{
	ASSERT( 0 != g_pModelLT );
	ASSERT( 0 != g_pClientFXMgr );
	ASSERT( 0 != m_pWeapon );
	...
	if( !m_OverheatFx.IsValid() )
	{
		if ( '\0' != m_pWeapon->szHeatPVOverheatFxName[ 0 ] )
		{ 
			// This is always the player view FX
			CLIENTFX_CREATESTRUCT fxInit( m_pWeapon->szHeatPVOverheatFxName, FXFLAG_LOOP | FXFLAG_REALLYCLOSE, m_hObject ); 
			g_pClientFXMgr->CreateClientFX( &m_OverheatFx, fxInit, false );
			if( m_OverheatFx.IsValid() ) 
			{
				m_OverheatFx.GetInstance()->Hide();
				m_OverheatFx.GetInstance()->SetPos( LTVector(0.0f,0.0f,0.0f), LTVector(0.0f,0.0f,0.0f) );
			}
		}
	}

	// Make sure our weapon heat information is up to date.
	UpdateWeaponHeat( );
}

// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CClientWeapon::RemoveOverheatFx
//
//	PURPOSE:	Destroys the overheat fx.
//
// ----------------------------------------------------------------------- //

void CClientWeapon::RemoveOverheatFx()
{
	if ( m_OverheatFx.IsValid() )
	{
		// The FX will run itself out and get deleted in the mgr but we should still null our ptr
		ASSERT( 0 != g_pClientFXMgr );
		g_pClientFXMgr->ShutdownClientFX( &m_OverheatFx );
	}
}
...
void CClientWeapon::Fire( bool bFire )
{
	...
	m_fFireTime = g_pLTClient->GetTime();
                                 ...
                                 LTVector *vFirePos ) const
{
		...
		( !g_pPlayerMgr->IsUsingExternalCamera() ) )
	{
		...
		if (CAutoTargetMgr::Instance().IsLockedOn())
		{
			// Fire at the closest node
			*vF = CAutoTargetMgr::Instance().GetTargetVector();
...
void CClientWeapon::ClientFire( LTVector const &vPath, LTVector const &vFirePos, LTVector *pObjectImpactPos, HOBJECT *pObjectImpact )
{
	...
	switch ( m_pAmmo->eType )
	{
		...
		default:
		{
			DebugCPrint(1,"ERROR in CClientWeapon::ClientFire().  Invalid Ammo Type!" );
		}
		break;
	}
}
...
bool ClientWeapon_VectorObjFilterFn(HOBJECT hTest, void *pUserData)
{
	...
	if ( (nObjUserFlags & USRFLG_HITBOX) != 0)
	{
		...
		if ( pBody )
		{
			//[jrg - 9/26/03] special case to handle planting bombs in contract jack...
			//					we don't want bodies to block bomb planting
			if (DT_GADGET_TIME_BOMB == pParams->m_pWeapon->GetAmmo()->eInstDamageType)
			{
				return false;
			}
			else
...
bool CClientWeapon::SpecialOverrideFire()
{
	...
	if( DT_GADGET_TIME_BOMB == m_pAmmo->eInstDamageType )
	{
		...
		{
			uint32 dwUsrFlgs;
			g_pCommonLT->GetObjectFlags( hTarget, OFT_User, dwUsrFlgs );
			...
			if( dwUsrFlgs & USRFLG_GADGET_BOMBABLE )
			{
				...
				if( IsTeamGameType() )
				{
					...
					CGadgetTargetFX *pGadget = dynamic_cast<CGadgetTargetFX*>(g_pGameClientShell->GetSFXMgr()->FindSpecialFX( SFX_GADGETTARGET_ID, hTarget ));
					if( !pGadget )
						return false;
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CClientWeapon::AddWeaponHeat
//
//	PURPOSE:	Adds to the weapon overheat.
//
// ----------------------------------------------------------------------- //
void CClientWeapon::AddWeaponHeat()
{
	WEAPON const *pWeaponData = GetWeapon();
	if( !pWeaponData )
		return;

	// Check if this weapon doesn't have any heat per shot.
	if( pWeaponData->fHeatFactor <= 0.0f )
		return;

	// Add in the heat factor.
	m_fHeatAmount += pWeaponData->fHeatFactor;
	if( m_fHeatAmount > 1.0f )
		m_fHeatAmount = 1.0f;

	m_fHeatTime = g_pLTClient->GetTime( );
}

// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CClientWeapon::UpdateWeaponHeat
//
//	PURPOSE:	Adds to the weapon overheat.
//
// ----------------------------------------------------------------------- //
void CClientWeapon::UpdateWeaponHeat( )
{
	// Check if our weapon isn't heating up.
	if( m_fHeatAmount <= 0.0f )
		return;
	...
	float fElapsedTime = fCurTime - m_fHeatTime;

	// Check if the weapon overheated and is still waiting to come back.
	if( m_fHeatAmount >= 1.0f )
	{
		// Check if we haven't waited long enough for the
		// overheat to cool off.
		if( fElapsedTime < pWeaponData->fHeatOverheatTime )
			return;

		// Overheat is done, remove it from time calc.
		fElapsedTime -= pWeaponData->fHeatOverheatTime;
		m_fHeatTime += pWeaponData->fHeatOverheatTime;
	}

	// Dissipate some heat.
	m_fHeatAmount -= fElapsedTime * pWeaponData->fHeatDissipationRate;
	m_fHeatAmount = Clamp( m_fHeatAmount, 0.0f, 1.0f );
	m_fHeatTime = fCurTime;
}