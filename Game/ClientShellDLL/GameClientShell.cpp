...
CGameClientShell::~CGameClientShell()
{
	...
	if( !m_bLaunchedServerApp )
		LaunchFromString( "endsplash.htm", NULL );
#endif // defined (_DEMO)
}
...
void CGameClientShell::InitSound()
{
	...
	{
		DebugCPrint(1,"CGameClientShell::InitSound() - 16 bit sound");
	}
	else
	{
		DebugCPrint(1,"CGameClientShell::InitSound() - 8 bit sound");
...
uint32 CGameClientShell::OnEngineInitialized(RMode *pMode, LTGUID *pAppGuid)
{
	...
	*pAppGuid = GAMEGUID;
	...
	if (pRegMgr->IsValid())
	{
		...
		nGameRuns++;
		pRegMgr->Set( "GameRuns", nGameRuns );
	}
	...
	{
		g_pClientMultiplayerMgr->SetModName( RETAIL_MOD_NAME );
	}	
...
void CGameClientShell::Update()
{
	...
	if (GetInterfaceMgr( )->Update())
	{
		...
		// brought up...
		if( IsMultiplayerGame() && GetPlayerMgr()->IsPlayerInWorld() )
		{
...
{
	bool bWasServerLoaded = IsServerLoaded();
	m_eSwitchingWorldsState = ( SwitchingWorldsState )pMsg->Readuint8( );
	bool bServerLoaded = IsServerLoaded();
...
void CGameClientShell::HandleMsgMultiplayerOptions( ILTMessage_Read* pMsg )
{
	...
	case eGameTypeDeathmatch:
		...
		pProfile->m_ServerGameOptions.GetDeathmatch().m_bWeaponsStay = bWeaponsStay;
		break;
	case eGameTypeTeamDeathmatch:
		...
		pProfile->m_ServerGameOptions.GetTeamDeathmatch().m_bWeaponsStay = bWeaponsStay;
		break;
	case eGameTypeDemolition:
		pProfile->m_ServerGameOptions.GetDemolition().m_nRunSpeed = nRunSpeed;
		pProfile->m_ServerGameOptions.GetDemolition().m_nRounds = nRounds;
		pProfile->m_ServerGameOptions.GetDemolition().m_bFriendlyFire = bFriendlyFire;
		pProfile->m_ServerGameOptions.GetDemolition().m_bWeaponsStay = bWeaponsStay;
		break;
	case eGameTypeDoomsDay:
		...
		pProfile->m_ServerGameOptions.GetDoomsday().m_bWeaponsStay = bWeaponsStay;
		break;
	}
...
void CGameClientShell::RenderCamera(bool bDrawInterface)
{
	...
	m_ClientFXMgr.UpdateAllActiveFX( IsMultiplayerGame() ? true : m_bMainWindowFocus );
...
void CGameClientShell::OnModelKey(HLOCALOBJ hObj, ArgList *pArgs)
{
	...
	if( !bResult )
	{
		g_pMoveMgr->GetVehicleMgr()->OnModelKey( hObj, pArgs );
	}
}
...
void CGameClientShell::OnPlaySound(PlaySoundInfo* pPSI)
{
	...
	if (!pFilter)
	{
		...
		DebugCPrint(1,"  couldn't find filter (%d)", pPSI->m_UserData);
		return;
	}
	...
	if (pFilter && pPSI->m_hSound)
	{
		...
		if (g_pSoundFilterMgr->IsDynamic(pFilter))
		{
			...
			{
				DebugCPrint(1,"ERROR in CGameClientShell::OnPlaySound()!");
				DebugCPrint(1,"  couldn't find a dynamic filter (%s)", pFilter->szName);
				return;
			}
		}
	...
	else
	{
		...
		if (pFilter)
		{
			DebugCPrint(1,"  Invalid sound associated with Filter: %s", pFilter->szName);
		}
		else
		{
			DebugCPrint(1,"  Invalid filter associated with FilterId: %d", pPSI->m_UserData);
		}
	}
#endif
}
...
void CGameClientShell::HandleWeaponPickup(uint8 nWeaponID, bool bSuccess, bool bActivatingPickup )
{
...
void CGameClientShell::HandleGearPickup(uint8 nGearId, bool bSuccess, bool bActivatingPickup )
{
...
void CGameClientShell::HandleModPickup(uint8 nModId, bool bSuccess, bool bActivatingPickup )
{
...
void CGameClientShell::HandleAmmoPickup(uint8 nAmmoId, int nAmmoCount, 
										bool bSuccess, bool bActivatingPickup,
...
// --------------------------------------------------------------------------- //
//
//	ROUTINE:	CGameClientShell::LaunchPatchUpdate
//
//	PURPOSE:	Launchers patchupdate given URL to patch file.
//
// --------------------------------------------------------------------------- //

bool CGameClientShell::LaunchPatchUpdate( char const* pszUrl )
{
	...
	if( !LaunchFromString( pszExe, pszUrl ))
		return false;
...
// --------------------------------------------------------------------------- //
//
//	ROUTINE:	CGameClientShell::LaunchFromString
//
//	PURPOSE:	Runs a start command.
//
// --------------------------------------------------------------------------- //
bool CGameClientShell::LaunchFromString( char const* pszFile, char const* pszParameters )
{
	RMode rMode;
	...
	if(( int )ShellExecute( NULL, "open", pszFile, pszParameters, NULL, SW_SHOWNORMAL ) <= 32 )
	{
...
// --------------------------------------------------------------------------- //
//
//	ROUTINE:	CGameClientShell::CreateMPMissionFile
//
//	PURPOSE:	Creates the mpmission file dynamically.
//
// --------------------------------------------------------------------------- //

void CGameClientShell::CreateMPMissionFile()
{
	char path[256];
