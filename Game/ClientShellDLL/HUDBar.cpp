...
void CHUDBar::SetForegroundColor(uint32 argbColor)
{
	// Set the color on the foreground polygons.
	g_pDrawPrim->SetRGBA(&m_Poly[0],argbColor);
	g_pDrawPrim->SetRGBA(&m_Poly[1],argbColor);
	g_pDrawPrim->SetRGBA(&m_Poly[2],argbColor);
}
