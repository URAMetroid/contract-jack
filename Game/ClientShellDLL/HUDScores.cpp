...
{	
	if (!m_nDraw && bDraw && bScreen && !m_bScreen)
...
void CHUDScores::UpdateLayout()
{
	...
	uint16 nCol4 = g_pLayoutMgr->GetInt(pTag,"PingColumn");

	m_nFrameWidth = (nCol1+nCol2+nCol3+nCol4) + 16;
	...
	if (!m_bControlsInited)
	{
		...
		for (int team = 0; team < kNumTeams; team++)
		{
			...
			m_Header[team].AddColumn(LoadTempString(IDS_SCORE_PING), nCol4, LTFALSE);
			...
			for (int i = 0; i < kMaxPlayers; i++)
			{
				...
				m_Columns[team][i].AddColumn("<Ping>", nCol4, LTTRUE);
