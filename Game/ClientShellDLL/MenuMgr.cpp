...
LTBOOL CMenuMgr::Init()
{
	...
	uint16 xpos = g_pLayoutMgr->GetInt(pTag,"BarTextOffset");
	...
	LTIntPt offset(xpos,(size.y-fontSize)/2);
	for (int i = 0; i < (int)m_MenuArray.size() ; i++)
	{
	...
	offset.x += m_MenuBar.GetBasePos().x;
	offset.y += m_MenuBar.GetBasePos().y;
	for (int i = (int)m_MenuArray.size()-1; i >= 0; i--)
	{
		CLTGUITextCtrl *pCtrl = (CLTGUITextCtrl *)m_MenuBar.GetControl(i);
		offset.x -= (pCtrl->GetBaseWidth()+nBarSpacing);
		pCtrl->SetBasePos(offset);
	}
