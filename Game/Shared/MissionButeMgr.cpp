...
#define MMGR_MISSION_SECRETS			"NumSecrets"
...
MISSION::MISSION()
{
	...
	nSelectedWeapon		= WMGR_INVALID_ID;
...
LTBOOL MISSION::Init(CButeMgr & buteMgr, char* aTagName)
{
	...
	nNumSecrets = (uint8)buteMgr.GetInt(aTagName, MMGR_MISSION_SECRETS,0);
...
int MISSION::BuildWeaponsList(char* pszStr, int* pArray, int nArrayLen)
{
	...
	if (strlen(pszStr))
	{
		...
		while (pWpnName && nNumWeapons < nArrayLen)
		{
			...
			{
                DebugCPrint(1,"Error in %s: Unknown weapon: %s", g_pMissionButeMgr->GetAttributeFile( ),pWpnName);
            }
...
int MISSION::BuildAmmoList(char *pszStr, int* pArray, int nArrayLen)
{
	...
	if (strlen(pszStr))
	{
		...
		while (pAmmoName && nNumAmmo < nArrayLen)
		{
			...
			{
                DebugCPrint(1,"Error in %s: Unknown ammo: %s", g_pMissionButeMgr->GetAttributeFile( ),pAmmoName);
			}
...
int MISSION::BuildModsList(char *pszStr, int* pArray, int nArrayLen)
{
	...
	if (strlen(pszStr))
	{
		...
		while (pModName && nNumMods < nArrayLen)
		{
			...
			{
                DebugCPrint(1,"Error in %s: Unknown weapon mod: %s", g_pMissionButeMgr->GetAttributeFile( ),pModName);
			}
