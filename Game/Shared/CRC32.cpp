...
uint32 CRC32::CalcFileCRC(const char *pFilename, char const* pszRealName)
{
	...
	// Add the file title to the crc value.
	char szTitle[MAX_PATH*2];
	_splitpath( pszRealName, NULL, NULL, szTitle, NULL );
	_strupr( szTitle );

	// Add the file title to the crc value.
	char const* pszFinger = szTitle;
	while( pszFinger[0] )
	{
		g_CRCTable.Calc( nAccumulator, ( uint8 )pszFinger[0] );
		pszFinger++;
	}
...
{
	return CalcFileCRC( pFilename, pFilename );
}
...
uint32 CRC32::CalcRezFileCRC(const char *pFilename)
{
	...
	// Add the file title to the crc value.
	char szTitle[MAX_PATH*2];
	_splitpath( pFilename, NULL, NULL, szTitle, NULL );
	_strupr( szTitle );
	char const* pszFinger = szTitle;
	while( pszFinger[0] )
	{
