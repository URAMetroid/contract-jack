...
class CSaveLoadMgr
{
	public : // Methods...
		...
		char const* const GetCheckpointSaveDir( const char *pProfile = NULL ) const
		{
			static char szSavePath[MAX_PATH];
			sprintf( szSavePath, "%s\\%s", GetRootSaveDir( pProfile ), CHECKPOINTSAVE_DIR );

			return szSavePath;
		}

		char const* const GetCheckpointSaveFile( const char *pProfile = NULL ) const
		{
			static char szSaveFile[MAX_PATH];
			sprintf( szSaveFile, "%s\\%s", GetRootSaveDir( pProfile ), CHECKPOINTSAVE_FILENAME );
