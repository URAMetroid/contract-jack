...
#define WMGR_WEAPONORDER_HOLSTERWEAPON		"HolsterWeapon"
...
#define WMGR_WEAPON_EFFECTIVEVECTORRANGE	"EffectiveVectorRange"
#define WMGR_WEAPON_VECTORHALFDAMAGEDIST	"VectorHalfDamageDist"
...
#define WMGR_WEAPON_USEAUTOTARGET			"UseAutoTarget"
#define WMGR_WEAPON_AUTOTARGETANGLE			"AutoTargetAngle"
#define WMGR_WEAPON_AUTOTARGETRANGE			"AutoTargetRange"
#define WMGR_WEAPON_SHOWCHOOSER				"ShowChooser"
#define WMGR_WEAPON_HEATFACTOR				"HeatFactor"
#define WMGR_WEAPON_HEATDISSIPATIONRATE		"HeatDissipationRate"
#define WMGR_WEAPON_HEATOVERHEATTIME		"HeatOverheatTime"
#define WMGR_WEAPON_HEATPVOVERHEATFXNAME	"HeatPVOverheatFxName"
...
#define WMGR_AMMO_HEATSEEKING		"HeatSeeking"
#define WMGR_AMMO_HEATSEEKINGRANGE	"HeatSeekingRange"
#define WMGR_AMMO_HEATSEEKINGANGLE	"HeatSeekingAngle"
#define WMGR_AMMO_HEATSEEKINGRATEOFTURN "HeatSeekingRateOfTurn"
...
#define WMGR_GLOBAL_TAG			"Global"
#define WMGR_GLOBAL_VECTORHALFDAMAGEDIST	"VectorHalfDamageDist"
#define WMGR_MULTI_TAG			"Multiplayer"
#define WMGR_MULTI_DEFAULTS		"DefaultWeapons"
#define WMGR_MULTI_SELECTED		"SelectedWeapon"
...
CWeaponMgr::CWeaponMgr()
{
	...
	m_fVectorHalfDamageDist = 1000.0f;

	m_pHolsterWeapon = NULL;
}
...
LTBOOL CWeaponMgr::Init(const char* szAttributeFile)
{
	...
	m_fVectorHalfDamageDist = Max( 1.0f, ( float )m_buteMgr.GetInt(WMGR_GLOBAL_TAG, WMGR_GLOBAL_VECTORHALFDAMAGEDIST, 1000 ));
	...
	m_pHolsterWeapon = GetWeapon( m_buteMgr.GetString( WMGR_WEAPONORDER_TAG, WMGR_WEAPONORDER_HOLSTERWEAPON, "" ));
	...
	m_buteMgr.GetString(WMGR_MULTI_TAG, WMGR_MULTI_SELECTED,"", szTmp, sizeof(szTmp));
	m_sMPSelected = szTmp;
...
LTBOOL CWeaponMgr::LoadOverrideButes( const char *szAttributeFile /*=WEAPON_DEFAULT_MULTI_FILE*/ )
{
	...
	// If the override bute file is different than the default
	// then record its name.  We'll use it for CRC'ing.
	if( stricmp( s_AttributeFile, szAttributeFile ) != 0 )
	{
		m_sOverrideBute = szAttributeFile;
	}
	else
	{
		m_sOverrideBute = "";
...
const char* CWeaponMgr::GetMPSelectedWeapon() const
{
	return m_sMPSelected.c_str();
}
...
WEAPON::WEAPON()
{
	...
	nEffectiveVectorRange		= 0;
	fVectorHalfDamageDist = 1000.0f;
	nRange				= 0;
	nVectorsPerRound	= 0;
	...
	eUseAutoTarget = AT_EASY;
	fAutoTargetAngle = 10.0f;
	fAutoTargetRange = 3200.0f;
	
	bShowChooser = true;

	fHeatFactor = 0.0f;
	fHeatDissipationRate = 1.0f;
	fHeatOverheatTime = 0.0f;
}
...
WEAPON::~WEAPON()
{
	...
	debug_deletea( szHeatPVOverheatFxName );
...
void WEAPON::InitMembers( CButeMgr &buteMgr, char *aTagName )
{
	...
	GetStringIfExist( buteMgr, aTagName, WMGR_WEAPON_HEATPVOVERHEATFXNAME, &szHeatPVOverheatFxName, WMGR_MAX_NAME_LENGTH );
	...
	nEffectiveVectorRange     = buteMgr.GetInt(aTagName, WMGR_WEAPON_EFFECTIVEVECTORRANGE, nEffectiveVectorRange );
	fVectorHalfDamageDist     = Max( 1.0f, ( float )buteMgr.GetInt(aTagName, WMGR_WEAPON_VECTORHALFDAMAGEDIST, ( int )g_pWeaponMgr->GetGlobalVectorHalfDamageDist( )));
	...
	int nUseAutoTarget = buteMgr.GetInt( aTagName, WMGR_WEAPON_USEAUTOTARGET, 1 );
	if( buteMgr.Success( ))
	{
		switch (nUseAutoTarget)
		{
		case 0:
			eUseAutoTarget = AT_NEVER;
			break;
		case 2:
			eUseAutoTarget = AT_ALWAYS;
			break;
		default:
			eUseAutoTarget = AT_EASY;
			break;
	...
	fAutoTargetAngle = (float)buteMgr.GetDouble( aTagName, WMGR_WEAPON_AUTOTARGETANGLE, fAutoTargetAngle );
	fAutoTargetRange = (float)buteMgr.GetDouble( aTagName, WMGR_WEAPON_AUTOTARGETRANGE, fAutoTargetRange );

	bShowChooser = !!(buteMgr.GetInt( aTagName, WMGR_WEAPON_SHOWCHOOSER, bShowChooser ));

	fHeatFactor = (float)buteMgr.GetDouble( aTagName, WMGR_WEAPON_HEATFACTOR, fHeatFactor );
	fHeatDissipationRate = (float)buteMgr.GetDouble( aTagName, WMGR_WEAPON_HEATDISSIPATIONRATE, fHeatDissipationRate );
	fHeatOverheatTime = (float)buteMgr.GetDouble( aTagName, WMGR_WEAPON_HEATOVERHEATTIME, fHeatOverheatTime );
}
...
AMMO::AMMO()
{
	...
	bHeatSeeking = false;
	fHeatSeekingRange = 2000.0f;
	fHeatSeekingAngle = 45.0f;
	fHeatSeekingRateOfTurn = 30.0f;
}
...
void AMMO::InitMembers( CButeMgr &buteMgr, char *aTagName )
{
	...
	bHeatSeeking = !!buteMgr.GetInt( aTagName, WMGR_AMMO_HEATSEEKING, bHeatSeeking );
	fHeatSeekingRange = ( float )buteMgr.GetInt( aTagName, WMGR_AMMO_HEATSEEKINGRANGE, ( int )fHeatSeekingRange );
	fHeatSeekingAngle = ( float )buteMgr.GetInt( aTagName, WMGR_AMMO_HEATSEEKINGANGLE, ( int )fHeatSeekingAngle );
	fHeatSeekingRateOfTurn = ( float )buteMgr.GetInt( aTagName, WMGR_AMMO_HEATSEEKINGRATEOFTURN, ( int )fHeatSeekingRateOfTurn );
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CWeaponMgr::GetFileCRC
//
//	PURPOSE:	Calculate the file crc.
//
// ----------------------------------------------------------------------- //

uint32 CWeaponMgr::GetFileCRC( )
{
	uint32 nFileCRC = CRC32::CalcRezFileCRC( s_AttributeFile );
	if( m_sOverrideBute.length( ) != 0 )
		nFileCRC += CRC32::CalcRezFileCRC( m_sOverrideBute.c_str( ));

	for( int i = 0; i < m_nNumWeapons; i++ )
	{
		// Add the pvweapon model to the crc.
		WEAPON* pWeapon = m_pWeaponList[i];
		if( pWeapon && pWeapon->szPVModel )
		{
			nFileCRC += CRC32::CalcRezFileCRC( pWeapon->szPVModel );
		}
	}
...
void CWeaponMgr::ReadWeapon(char const* szString, uint8 & nWeaponId, uint8 & nAmmoId)
{
	if( !szString || !szString[0] )
		return;

	char szWeaponName[256];
	LTStrCpy( szWeaponName, szString, ARRAY_LEN( szWeaponName ));

	// Get the weapon name...

	WEAPON const *pWeapon = GetWeapon(strtok(szWeaponName,","));
	if (pWeapon)
	{
		nWeaponId = pWeapon->nId;
	}
