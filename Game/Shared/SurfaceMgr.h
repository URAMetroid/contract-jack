...
#define SRF_MAX_VEHICLE_TYPES		6
...
struct SURFACE
{
	...
	// Class to hold data specific to vehicles...
	struct VehicleType
	{
		VehicleType()
		:	fVelMult	( 1.0f )
		...
		CButeListReader	blrSnds;
		float			fVelMult;
		std::string		sImpactFX;
	};

	VehicleType *GetVehicleType( uint8 i )
	{
		if( (i >= 0) && (i < SRF_MAX_VEHICLE_TYPES) )
		{
			return lstVehicleTypes[i];
		}
...
private:
	...
	typedef std::vector< VehicleType* > VehicleTypeList;
	VehicleTypeList		lstVehicleTypes;
