...
//	int             nNameId;
	...
	// Heat seeking
	bool	bHeatSeeking;
	float	fHeatSeekingRange;
	float	fHeatSeekingAngle;
	float	fHeatSeekingRateOfTurn;
...
struct WEAPON
{
	...
	int		nEffectiveVectorRange;
	float	fVectorHalfDamageDist;
	int		nRange;
	LTVector vRecoil;
	...
	//auto-targetting info
	AutoTargetType	eUseAutoTarget;
	float	fAutoTargetAngle;
	float	fAutoTargetRange;

	// Shows up in chooser.
	bool	bShowChooser;

	// Heat factor per shot fired.
	float	fHeatFactor;
	float	fHeatDissipationRate;
	float	fHeatOverheatTime;
	char	*szHeatPVOverheatFxName;
...
class CWeaponMgr : public CGameButeMgr
{
	...
	public :
        ...
        void        ReadWeapon(char const* szString, uint8 & nWeaponId, uint8 & nAmmoId);
#endif // _CLIENTBUILD
		...
		const char* GetMPSelectedWeapon() const;

		float GetGlobalVectorHalfDamageDist( ) const { return m_fVectorHalfDamageDist; }

		WEAPON const* GetHolsterWeapon( ) { return m_pHolsterWeapon; }
	...
	protected :
		...
		// The weapon for the holster weapon.
		WEAPON const*		m_pHolsterWeapon;
		...
		std::string		m_sMPSelected;

		float				m_fVectorHalfDamageDist;

		std::string			m_sOverrideBute;
};
