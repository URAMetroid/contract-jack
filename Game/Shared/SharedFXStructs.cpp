...
void CHARCREATESTRUCT::Write(ILTMessage_Write *pMsg)
{
	...
	pMsg->Writeuint8( pChassisPieceBute ? pChassisPieceBute->m_nIndex : CHASSISPIECEBUTE_INVALID );

	pMsg->Writeuint8( ePlayerPhysicsModel );
	pMsg->Writeuint8( nVehicleId );
}
...
void CHARCREATESTRUCT::Read(ILTMessage_Read *pMsg)
{
	...
	uint8 nChassisPieceButeIndex = pMsg->Readuint8( );
	if( nChassisPieceButeIndex != CHASSISPIECEBUTE_INVALID )
		pChassisPieceBute = ChassisButeMgr::Instance( ).GetChassisPieceButeList( )[nChassisPieceButeIndex];
	else
		pChassisPieceBute = NULL;
	ePlayerPhysicsModel = (PlayerPhysicsModel)pMsg->Readuint8();
	nVehicleId			= pMsg->Readuint8();
