...
#define MODELBMGR_SKELETON				"Skeleton"
	...
	#define MODELBMGR_SKELETON_NODE_AUTOTARGET					"Node%dAutoTarget"
...
LTBOOL CModelButeMgr::Init(const char* szAttributeFile)
{
	...
	for ( int iSkeleton = 0 ; iSkeleton < m_cSkeletons ; iSkeleton++ )
	{
		...
		for ( int iNode = 0 ; iNode < m_aSkeletons[iSkeleton].m_cNodes ; iNode++ )
		{
			...
			// Get the node's auto target flag
			sprintf( s_aAttName, MODELBMGR_SKELETON_NODE_AUTOTARGET, iNode );
			if (cAutoTargetCount < MAX_MODELBMGR_AUTOTARGET_NODES)
			{
				
				m_aSkeletons[iSkeleton].m_aNodes[iNode].m_bAutoTarget = !!m_buteMgr.GetInt( s_aTagName, s_aAttName, 0 );
				if (m_aSkeletons[iSkeleton].m_aNodes[iNode].m_bAutoTarget)
					cAutoTargetCount++;
			}
		}
	}
...
bool CModelButeMgr::GetSkeletonNodeAttachSpears( ModelSkeleton eModelSkeleton, ModelNode eModelNode )
{
	_ASSERT(eModelSkeleton >= 0 && eModelSkeleton < m_cSkeletons);
	_ASSERT(eModelNode >= 0 && eModelNode < m_aSkeletons[eModelSkeleton].m_cNodes);
	if (eModelSkeletonInvalid == eModelSkeleton) return false;

	return m_aSkeletons[eModelSkeleton].m_aNodes[eModelNode].m_bAttachSpears;
}

bool CModelButeMgr::GetSkeletonNodeAutoTarget( ModelSkeleton eModelSkeleton, ModelNode eModelNode )
{
	_ASSERT(eModelSkeleton >= 0 && eModelSkeleton < m_cSkeletons);
	_ASSERT(eModelNode >= 0 && eModelNode < m_aSkeletons[eModelSkeleton].m_cNodes);
	if (eModelSkeletonInvalid == eModelSkeleton) return false;

	return m_aSkeletons[eModelSkeleton].m_aNodes[eModelNode].m_bAutoTarget;
}
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CModelButeMgr::GetFileCRC
//
//	PURPOSE:	Calculate the filecrc.
//
// ----------------------------------------------------------------------- //

uint32 CModelButeMgr::GetFileCRC( )
{
	// CRC the attribute file
	uint32 nFileCRC = CRC32::CalcRezFileCRC(GetAttributeFile( ));
	...
	for ( int iDMModel = 0 ; iDMModel < m_cDMModels ; iDMModel++ )
	{
		char const* pszModelFilename = GetMultiModelFilename( m_aDMModels[iDMModel] );
		nFileCRC += CRC32::CalcRezFileCRC(pszModelFilename);
		aModelCRC[iDMModel] = 1;
	}

	for ( int iTModel = 0 ; iTModel < m_cTeamModels ; iTModel++ )
	{
		if( aModelCRC[iTModel] == 0 )
		{
			char const* pszModelFilename = GetMultiModelFilename( m_aTeamModels[iTModel] );
			nFileCRC += CRC32::CalcRezFileCRC(pszModelFilename);
			aModelCRC[iTModel] = 1;
		}
	}
