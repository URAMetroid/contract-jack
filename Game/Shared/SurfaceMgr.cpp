...
#define SRFMGR_SURFACE_VEHICLETYPESND			"VehicleType%d_Snd"
#define SRFMGR_SURFACE_VEHICLETYPEVELMULT		"VehicleType%d_VelMult"
#define SRFMGR_SURFACE_VEHICLETYPEIMPACTFX		"VehicleType%d_ImpactFX"
...
SURFACE::SURFACE()
{
	...
	lstVehicleTypes.reserve( SRF_MAX_VEHICLE_TYPES );
}

SURFACE::~SURFACE( )
{
	VehicleTypeList::iterator iter = lstVehicleTypes.begin();
	while( iter != lstVehicleTypes.end() )
	{
		debug_delete( *iter );
		iter = lstVehicleTypes.erase( iter );
	}

	lstVehicleTypes.clear();
}
...
LTBOOL SURFACE::Init(CButeMgr & buteMgr, char* aTagName)
{
	...
	// Read the vehiceltype data...

	for( i = 0; i < SRF_MAX_VEHICLE_TYPES; ++i )
	{
		VehicleType *pVT = debug_new( VehicleType );

		sprintf( s_aAttName, SRFMGR_SURFACE_VEHICLETYPESND, i );
		pVT->blrSnds.Read( &buteMgr, aTagName, s_aAttName, SRF_MAX_FILE_PATH );

		sprintf( s_aAttName, SRFMGR_SURFACE_VEHICLETYPEVELMULT, i );
		pVT->fVelMult = (float)buteMgr.GetDouble( aTagName, s_aAttName, 1.0f );

		sprintf( s_aAttName, SRFMGR_SURFACE_VEHICLETYPEIMPACTFX, i );
		pVT->sImpactFX = buteMgr.GetString( aTagName, s_aAttName, "" );

		lstVehicleTypes.push_back( pVT );
	}
